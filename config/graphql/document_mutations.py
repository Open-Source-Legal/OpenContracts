...
class RetryDocumentProcessing(graphene.Mutation):
    ...
    def mutate(root, info, document_id) -> "RetryDocumentProcessing":
        ...
        try:
            ...
            # Document.user_can encapsulates is_public + creator + guardian.
            if not document.user_can(info.context.user, PermissionTypes.READ):
                return RetryDocumentProcessing(
                    ok=False, message="Document not found", document=None
                )
            ...
            # Check user has UPDATE permission (superuser/creator handled inside)
            if not document.user_can(info.context.user, PermissionTypes.UPDATE):
                return RetryDocumentProcessing(
                    ok=False,
...
class RestoreDocumentToVersion(graphene.Mutation):
    ...
    def mutate(root, info, document_id, corpus_id) -> "RestoreDocumentToVersion":
        ...
        try:
            ...
            if not old_version.user_can(user, PermissionTypes.UPDATE):
                return RestoreDocumentToVersion(
                    ok=False,
                    message=not_found_msg,
                    document=None,
