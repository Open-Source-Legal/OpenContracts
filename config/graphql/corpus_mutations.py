...
def _mutate_StartCorpusFork(payload_cls, root, info, corpus_id, preferred_embedder=None):
...
def _mutate_RunCorpusAction(payload_cls, root, info, corpus_action_id: str, document_id: str):
    ...
    return mutate(root, info, corpus_action_id=corpus_action_id, document_id=document_id)
...
def _mutate_AddTemplateToCorpus(payload_cls, root, info, template_id: str, corpus_id: str):
...
def _mutate_UpdateArtifact(payload_cls, root, info, slug, title=None, subtitle=None, byline=None, config=None):
    ...
    return mutate(
        root, info, slug=slug, title=title, subtitle=subtitle, byline=byline, config=config
    )
