...
class RunCorpusEnrichmentMutation:
    ...
    def analyses(self, info: strawberry.Info) -> Optional[list[Optional[Annotated["AnalysisType", strawberry.lazy("config.graphql_new.extract_types")]]]]:
        return resolve_django_list(self, info, getattr(self, "analyses"), "AnalysisType")
    partial: Optional[bool] = strawberry.field(name="partial", description='True when some requested jobs dispatched but others failed (e.g. enrichment started but the crawl could not be dispatched). Only meaningful when ``ok`` is True; lets callers surface the non-fatal ``message`` without coupling to its text.')
