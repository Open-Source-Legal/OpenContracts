...
class CorpusActionType(Node):
    ...
    fieldset: Optional[Annotated["FieldsetType", strawberry.lazy("config.graphql_new.extract_types")]] = strawberry.field(name="fieldset")
    analyzer: Optional[Annotated["AnalyzerType", strawberry.lazy("config.graphql_new.extract_types")]] = strawberry.field(name="analyzer")
    agent_config: Optional["AgentConfigurationType"] = strawberry.field(name="agentConfig", description='Optional agent configuration for persona/tool defaults. Not required for agent actions — task_instructions alone is sufficient.')
    ...
    run_on_all_corpuses: bool = strawberry.field(name="runOnAllCorpuses")
    source_template: Optional["CorpusActionTemplateType"] = strawberry.field(name="sourceTemplate")
...
class CorpusActionExecutionType(Node):
    ...
    created: datetime.datetime = strawberry.field(name="created")
    modified: datetime.datetime = strawberry.field(name="modified")
    corpus_action: "CorpusActionType" = strawberry.field(name="corpusAction", description='The corpus action configuration that was executed')
    document: Optional[Annotated["DocumentType", strawberry.lazy("config.graphql_new.document_types")]] = strawberry.field(name="document", description='The document this action was executed on (null for thread-based actions)')
    conversation: Optional[Annotated["ConversationType", strawberry.lazy("config.graphql_new.conversation_types")]] = strawberry.field(name="conversation", description='The thread that triggered this execution (for thread-based actions)')
    message: Optional[Annotated["MessageType", strawberry.lazy("config.graphql_new.conversation_types")]] = strawberry.field(name="message", description='The message that triggered this execution (for NEW_MESSAGE trigger)')
    corpus: Annotated["CorpusType", strawberry.lazy("config.graphql_new.corpus_types")] = strawberry.field(name="corpus", description='Denormalized corpus reference for fast queries')
    ...
    queued_at: datetime.datetime = strawberry.field(name="queuedAt", description='When the execution was queued (set explicitly for bulk_create)')
    started_at: Optional[datetime.datetime] = strawberry.field(name="startedAt", description='When execution actually started')
    completed_at: Optional[datetime.datetime] = strawberry.field(name="completedAt", description='When execution completed (success or failure)')
    ...
    agent_result: Optional["AgentActionResultType"] = strawberry.field(name="agentResult", description='Detailed agent result (for agent actions only)')
    extract: Optional[Annotated["ExtractType", strawberry.lazy("config.graphql_new.extract_types")]] = strawberry.field(name="extract", description='Extract created (for fieldset actions only)')
    analysis: Optional[Annotated["AnalysisType", strawberry.lazy("config.graphql_new.extract_types")]] = strawberry.field(name="analysis", description='Analysis created (for analyzer actions only)')
...
class AgentConfigurationType(Node):
    ...
    badge_config: JSONString = strawberry.field(name="badgeConfig", description="Visual config: {'icon': 'bot', 'color': '#4A90E2', 'label': 'AI Assistant'}")
    ...
    corpus: Optional[Annotated["CorpusType", strawberry.lazy("config.graphql_new.corpus_types")]] = strawberry.field(name="corpus", description='Corpus this agent belongs to (if scope=CORPUS)')
    is_active: bool = strawberry.field(name="isActive", description='Whether this agent is active and can be used')
    @strawberry.field(name="myPermissions")
    def my_permissions(self, info: strawberry.Info) -> Optional[GenericScalar]:
        return core_permissions.resolve_my_permissions(self, info)
    @strawberry.field(name="isPublished")
...
class AgentActionResultType(Node):
    ...
    created: datetime.datetime = strawberry.field(name="created")
    modified: datetime.datetime = strawberry.field(name="modified")
    corpus_action: "CorpusActionType" = strawberry.field(name="corpusAction", description='The corpus action that triggered this execution')
    document: Optional[Annotated["DocumentType", strawberry.lazy("config.graphql_new.document_types")]] = strawberry.field(name="document", description='The document this action was run on (null for thread-based actions)')
    conversation: Optional[Annotated["ConversationType", strawberry.lazy("config.graphql_new.conversation_types")]] = strawberry.field(name="conversation", description='Conversation record containing the full agent interaction')
    triggering_conversation: Optional[Annotated["ConversationType", strawberry.lazy("config.graphql_new.conversation_types")]] = strawberry.field(name="triggeringConversation", description='Thread that triggered this agent action (for thread-based triggers)')
    triggering_message: Optional[Annotated["MessageType", strawberry.lazy("config.graphql_new.conversation_types")]] = strawberry.field(name="triggeringMessage", description='Message that triggered this agent action (for NEW_MESSAGE trigger)')
    @strawberry.field(name="status")
...
class CorpusActionTemplateType(Node):
    ...
    def description(self, info: strawberry.Info) -> str:
        return coerce_str(getattr(self, "description", None))
    agent_config: Optional["AgentConfigurationType"] = strawberry.field(name="agentConfig", description='Optional agent configuration for persona/tool defaults.')
    @strawberry.field(name="preAuthorizedTools")
    def pre_authorized_tools(self, info: strawberry.Info) -> Optional[list[Optional[str]]]:
        kwargs = strip_unset({})
    ...
    is_active: bool = strawberry.field(name="isActive", description='Whether this template appears in the Action Library for users to add.')
    disabled_on_clone: bool = strawberry.field(name="disabledOnClone", description='If True, cloned actions start disabled (user must opt-in).')
    sort_order: int = strawberry.field(name="sortOrder", description='Display ordering in template lists.')
...
class CorpusActionTrailStatsType:
    total_executions: Optional[int] = strawberry.field(name="totalExecutions")
    completed: Optional[int] = strawberry.field(name="completed")
    failed: Optional[int] = strawberry.field(name="failed")
    running: Optional[int] = strawberry.field(name="running")
    queued: Optional[int] = strawberry.field(name="queued")
    skipped: Optional[int] = strawberry.field(name="skipped")
    avg_duration_seconds: Optional[float] = strawberry.field(name="avgDurationSeconds")
    fieldset_count: Optional[int] = strawberry.field(name="fieldsetCount")
    analyzer_count: Optional[int] = strawberry.field(name="analyzerCount")
    agent_count: Optional[int] = strawberry.field(name="agentCount")
...
class AvailableToolType:
    ...
    requiresCorpus: bool = strawberry.field(name="requiresCorpus", description='Whether this tool requires a corpus context')
    requiresApproval: bool = strawberry.field(name="requiresApproval", description='Whether this tool requires user approval before execution')
...
class ToolParameterType:
    ...
    def description(self, info: strawberry.Info) -> str:
        return coerce_str(getattr(self, "description", None))
    required: bool = strawberry.field(name="required", description='Whether the parameter is required')
