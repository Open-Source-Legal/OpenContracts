...
**OrcaRouter** is the one exception to the env-fallback string: pydantic-ai has no native `orcarouter:` provider prefix, so a bare `"orcarouter:..."` spec would raise "Unknown model". Instead `build_agent_model()` always builds a concrete OpenAI-compatible model for OrcaRouter — using DB-configured credentials when present, otherwise `ORCAROUTER_API_KEY` and the default endpoint `https://api.orcarouter.ai/v1` (see `opencontractserver/llms/model_factory.py::_construct_orcarouter_model`). This keeps the System Settings LLM picker safe: choosing an `orcarouter:` model can never produce an unresolvable spec. With no key configured an inert placeholder is sent (and a warning logged) — never `None`, which would let the OpenAI client fall back to `OPENAI_API_KEY` and send the install's OpenAI secret to the gateway. OrcaRouter is driven over chat completions only, so Responses-API-only families (pitfall #20) are not usable through it with tools; the factory warns when one is selected.

OrcaRouter context windows are **resolved dynamically**, not listed in `MODEL_CONTEXT_WINDOWS`: each agent build calls `opencontractserver/llms/orcarouter_context.py::refresh_orcarouter_context_windows`, which fetches the gateway's OpenAI-compatible `GET {base_url}/models` (TTL-cached, short timeout, no redirects, never raises) and records each entry's `context_length` / `context_window` / `max_context_length`. `get_context_window_for_model` reads only that cache (no I/O on the async path) and falls back to `ORCAROUTER_FALLBACK_CONTEXT_WINDOW` (`opencontractserver/constants/context_guardrails.py`) until a listing is fetched or when it omits the model.

Any failure to build a credentialed model degrades to the env-fallback string, so a misconfiguration can never take the chat path down — except for OrcaRouter, whose bare spec is unresolvable, so its construction errors are re-raised rather than hidden behind a later "Unknown model". The factory is invoked at every `make_pydantic_ai_agent` call site (document, corpus, and structured-output agents, plus the memory-curation tasks); it performs ORM access, so async call sites use the `abuild_agent_model()` wrapper.
