...
Selected resource arguments also pass `_check_target_permissions` using the same actor. This covers positional, default, and forwarded keyword arguments. Note/annotation writers require the selected target's existing CRUD permission; thread reads and posting require model READ. The five moderation actions retain their service-specific role checks. Malformed arguments return normal tool errors; permission and approval exceptions still propagate.
