...
| `BaseService.filter_visible_qs(queryset, user, *, request=None, **kwargs)` | Same intent as `filter_visible`, but starts from an **existing queryset or related manager** — chains the queryset's own `visible_to_user` so the visibility predicate stays in the same `WHERE` expression tree as any prior `.filter(...)` clauses (no correlated `pk__in` subquery). Use this in `DjangoObjectType.get_queryset` overrides and reverse-relation field resolvers. Requires a model whose *queryset* carries `visible_to_user` (`PermissionManager`, `PermissionedTreeQuerySet`, per-app QuerySets); models that keep the bare `BaseVisibilityManager` (`UserExport`, `UserImport`, ...) fail closed with `TypeError` — use `filter_visible(Model, user).filter(pk__in=related.values("pk"))` for those (see `config/graphql/user_types.py::_filter_visible_transfers`). | `QuerySet` |
