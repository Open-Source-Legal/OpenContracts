...
- **Sidecar + upversion intersection test** (`opencontractserver/tests/test_sidecar_import.py` — new `TestSidecarUpversioning` class). Pins the contract that re-importing a zip with a sidecar at an already-occupied corpus path (a) creates a new `Document` in the same `version_tree_id` with `parent` pointing at the prior version and `DocumentPath.version_number` incremented; (b) attaches the new sidecar's title / description / text annotations / doc-level labels to the **new** `Document`; (c) leaves the prior version's annotations attached to the now-non-current `Document` (no migration, no deletion); and (d) the current-path query (`DocumentPath.is_current=True, is_deleted=False`) resolves only to the new version's annotations. Existing upversion tests in `test_zip_import_integration.py::TestDocumentUpversioning` covered the path/version-tree mechanics; this test pins the sidecar-payload-meets-collision case that they didn't exercise.
