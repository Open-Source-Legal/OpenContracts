...
- **LLM configuration system for extracts (per-column model selection)**: New superuser-managed registry of LLM providers, models, and API keys, mirroring the existing `PipelineSettings` pattern. Hardcoded `"openai:gpt-4o-mini"` in `opencontractserver/tasks/data_extract_tasks.py:277` is replaced with a resolution layer (`opencontractserver.llms.config_service.resolve_model_for_column`) that consults the column's `preferred_llm_model` FK, falls back to the system default, and fails the cell with a descriptive message when neither is usable.
  - **Encryption helper** (`opencontractserver/utils/encryption.py`): Fernet + PBKDF2-HMAC-SHA256 helpers extracted from `PipelineSettings._derive_key/get_secrets/set_secrets` so both singletons share one audited implementation. `PipelineSettings` refactored to delegate to the new helper (`opencontractserver/documents/models.py:1247–1290`); behaviour and storage format unchanged.
  - **Provider registry** (`opencontractserver/llms/providers/`): code-defined `BaseLLMProvider` subclasses auto-discovered at first registry access. Ships with OpenAI, Anthropic, Google (Generative Language), Azure OpenAI, and Ollama. Each provider declares its `pydantic_ai_prefix`, credential schema, default-model autocomplete suggestions, and the env-var overrides used to inject credentials into pydantic-ai at invocation time.
  - **Database models** (`opencontractserver/llms/models.py`, migration `opencontractserver/llms/migrations/0001_initial.py`): `LLMConfigSettings` singleton (encrypted credentials + non-secret per-provider config + default-model FK) and `LLMModel` (admin-curated rows with capability flags and `is_available`/`pydantic_ai_string` helpers). Singleton enforces `pk=1` via CheckConstraint and is invalidated from cache on save.
  - **Column FK** (`opencontractserver/extracts/models.py`, migration `opencontractserver/extracts/migrations/0029_column_preferred_llm_model.py`): nullable `preferred_llm_model` with `on_delete=SET_NULL` so deleting a model lets dependent columns fall back to the system default rather than cascading.
  - **GraphQL surface**: `llmProviders` (login-required), `availableLlmModels` (login-required, filtered by enabled + provider configured), `llmConfigSettings`/`llmModels` (superuser only). Mutations `updateLlmProviderCredentials`, `deleteLlmProviderCredentials`, `createLlmModel`, `updateLlmModel`, `deleteLlmModel`, `setDefaultLlmModel` are superuser-only and rate-limited to 30 req/min via `RateLimits.WRITE_LIGHT`. Secret values are NEVER returned in any response payload — only `secretFieldsSet` (list of field names) and `isConfigured` (boolean).
  - **Column GraphQL fields** (`config/graphql/extract_types.py`, `config/graphql/extract_mutations.py`): `ColumnType.preferredLlmModel` resolves the row even when unavailable (so the editor can render the warning state instead of silently dropping the user's intent). `CreateColumn` and `UpdateColumnMutation` accept `preferredLlmModelId: ID` (empty string clears the preference). The pre-existing dead `language_model_id` parameter in `UpdateColumnMutation` was removed.
  - **Frontend admin tab** (`/admin/llms`, `frontend/src/components/admin/LLMConfigManagement.tsx`): three-section page (Providers / Models / Default) accessible from the Admin Settings landing card. Provider cards expose configure/clear/add-model actions; model rows have enable toggles and per-row availability state; default-model dropdown is sourced from `availableLlmModels`.
  - **Column-editor model picker** (`frontend/src/components/widgets/modals/CreateColumnModal.tsx`): "Language Model" dropdown sourced from `availableLlmModels`, defaulting to "Use system default". Columns whose chosen model becomes unavailable still render the row with a yellow warning banner so the user can keep or change the selection without surprise.
  - **Tests** (`opencontractserver/tests/test_llm_config.py`): singleton + Fernet round-trip, `LLMModel` validation against the registry, `resolve_model_for_column` fallback chain (default / preferred / unavailable / disabled), `applied_environment` context-manager restoration, GraphQL superuser gating, secret invisibility in payloads, end-to-end provider-credentials → enabled-model → default-set flow, and `Column.preferred_llm_model` `SET_NULL` round-trip.
  - **Note**: Same `SECRET_KEY` rotation caveat applies as for `PipelineSettings` — encrypted credentials become unrecoverable after rotation. Export via `LLMConfigSettings.get_instance().get_secrets()` before rotating.
