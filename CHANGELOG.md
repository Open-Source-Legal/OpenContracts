...
- **Phase D — IDOR-safe `get_for_user_or_none(model_cls, pk, user)` helper + mutation migration** (`opencontractserver/utils/permissioning.py`, `opencontractserver/tests/permissioning/test_get_for_user_or_none.py`, plus the affected `config/graphql/*_mutations.py` files; closes #1658). Adds a generic non-corpus-scoped lookup helper that returns `None` whenever the pk doesn't exist OR the caller can't READ the row, so mutation call sites render a single unified `"<resource> not found or you don't have permission..."` message and stop leaking object existence via differential error text (CLAUDE.md "IDOR Prevention" — formerly `Model.objects.get(pk=X)` raising `DoesNotExist` vs a separate `user_has_permission_for_obj` permission denial returned distinguishable error states). Helper is generic over the model TypeVar so callers keep concrete `.fieldset` / `.is_public` / etc. attribute typing across the gate, and catches `ValueError`/`TypeError` from garbage pk inputs so untrusted GraphQL ids can't bubble up as 500s.
  - **Migrated** (non-corpus-scoped sites; `get_for_user_or_none`): `extract_mutations.py` — `UpdateExtractMutation` (Extract / Corpus / Fieldset lookups) and `CreateExtractIteration` (source Extract). `analysis_mutations.py` — `DeleteAnalysisMutation` (previously raised three different exception types depending on existence / read-permission / delete-permission). `agent_mutations.py` — `CreateAgentConfigurationMutation`, `UpdateAgentConfigurationMutation`, `DeleteAgentConfigurationMutation`. `corpus_mutations.py` — `UpdateCorpusMutation` preferred_embedder pre-check (previously leaked "this corpus has documents" via the early-exit), `UpdateCorpusDescription`, `DeleteCorpusMutation` `is_personal` pre-check, `StartCorpusFork`, `ReEmbedCorpus`, `CreateCorpusAction`, `AddTemplateToCorpus`, `ToggleCorpusMemory`, `AddDocumentsToCorpus`, `RemoveDocumentsFromCorpus`. `badge_mutations.py` — `AwardBadgeMutation` Badge / Corpus lookups. `label_mutations.py` — `DeleteMultipleLabelMutation` (creator-based permissions on `AnnotationLabel`). `conversation_mutations.py` — `CreateThreadMutation` (Corpus, Document), `CreateThreadMessageMutation`, `ReplyToMessageMutation`, `DeleteConversationMutation`, `DeleteMessageMutation`. `document_mutations.py` — `RetryDocumentProcessing`. `document_relationship_mutations.py` — `CreateDocumentRelationship` (Corpus + source/target Document), `UpdateDocumentRelationship` (Corpus).
  - **Deliberately retained bare `Model.objects.get`** at three sites because the helper's `visible_to_user` filter (READ codename) would lock out legitimate sharing workflows that grant a non-READ permission directly: `SetCorpusVisibility` (PERMISSION-without-READ — pinned by `test_user_with_permission_perm_can_change_visibility`; the malformed-id path is also pinned by `test_invalid_id_format_returns_error`), `CreateLabelForLabelsetMutation` / `RemoveLabelsFromLabelsetMutation` (UPDATE-without-READ — pinned by `test_non_owner_with_explicit_update_permission_can_create` / `test_remove_labels_allows_non_owner_with_explicit_update_permission`), and the `AwardBadgeMutation` User recipient lookup (`UserProfileManager.visible_to_user` has no superuser branch — would prevent superuser awarders from awarding to private-profile users). Inline comments at each site record the contract.
