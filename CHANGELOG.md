...
- **Auth0 JWT validation simplified and audit-logging restored** (PR #1499): the Auth0 stack carried a layer of debug logging from the original integration that has long since outlived its usefulness. `config/graphql_auth0_auth/utils.py`, `config/graphql_auth0_auth/backends.py`, `config/jwt_utils.py`, `config/admin_auth/backends.py`, `config/websocket/middleware.py`, and `opencontractserver/users/tasks.py` were trimmed of token-prefix logs, function-entry/exit traces, and redundant try/except wrappers; `get_payload()` now catches the `jwt.InvalidTokenError` umbrella instead of enumerating each subclass. `_parse_boolean_claim()` is now strict — only JSON booleans and canonical `"true"`/`"false"` strings are accepted, so a misconfigured Auth0 Action that emits `"yes"` or `1` fails closed instead of silently coercing. All outbound Auth0 HTTP calls in `users/tasks.py` now carry a 10-second `timeout=` (no more indefinite worker hangs); `sync_remote_user()` reuses a still-live token row instead of deleting and recreating; `apply_data_to_user()` uses `.get()` with defaults; token persistence runs inside `transaction.atomic()` with `select_for_update()` to defuse the concurrent-fetch race. `Auth0RemoteUserJSONWebTokenBackend` re-raises both `JSONWebTokenExpired` and `JSONWebTokenError` so GraphQL surfaces a structured error instead of silently downgrading to anonymous; `Auth0AdminBackend.authenticate()` is now an explicit no-op with a docstring explaining that token verification belongs in the view, not the backend. Migration `users/migrations/0027_drop_auth0apitoken_response.py` drops the unused `Auth0APIToken.auth0_Response` column. Frontend: `useAnnotationImages.tsx` now sends RFC 6750 `Authorization: Bearer <token>` (DRF accepts both `JWT` and `Bearer` via `config/rest_jwt_auth.py:48`); `errorLink.ts` prefers the structured `extensions.code in {JSONWebTokenExpired, TOKEN_EXPIRED, tokenExpired}` over message-string matching for expired-token detection; `useNavMenu.requestLogout` clears the `oc_has_authenticated` localStorage hint so the next visit takes the fast first-time-visitor path in AuthGate; `index.tsx` is no longer noisy with `console.log`. Audit-trail INFO logs were retained on `is_staff`/`is_superuser` flips in `sync_admin_claims_from_payload()` (rate-limited via `ADMIN_CLAIMS_CACHE_TTL`), the explicit `RSAPublicKey` runtime check (replacing `assert`) survives `python -O`, and `ensure_valid_auth0_token` now invokes `get_new_auth0_token.run()` synchronously in-process instead of `delay().get()`, which would deadlock under a saturated prefork pool. New tests in `opencontractserver/tests/test_user_auth0_tasks.py` cover the simplified task pipeline; `frontend/src/graphql/errorLink.test.ts` gains four cases for the new `extensions.code` branch.
