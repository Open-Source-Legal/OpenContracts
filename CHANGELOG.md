...
- **Backend test suite speedup — Phase A1 + B1** (plan in `docs/refactor_plans/2026-05-23-test-suite-speedup-A1-B1.md`). Targets the two highest-ROI levers from a CI runtime audit of `gh run view 26334920376` (47-min wall, 41-min pytest step): coverage on Python 3.11 measured at **+156% over the no-cov baseline** on the hot WebSocket / extract / structured-response files, and `--dist loadscope` pinned classes like `TestStructuredResponseAPI` (843 s / 27 tests) to a single worker — setting a ~14 min serial-floor for the suite. Combined local A/B (plan §6) showed a **4× speedup** on the hot subset (19:34 → 4:50) from these two changes alone.
  ...
  - **`pytest --dist worksteal`** (`.github/workflows/backend.yml`). Replaces `--dist loadscope` for the backend test step. Worksteal redistributes test items across idle workers, freeing the previously class-pinned heavy classes (e.g. `TestStructuredResponseAPI`'s 27 tests can now fan out across the full worker pool). The conftest auto-tag below preserves correctness for classes that genuinely need class binding.
  - **Auto-tag class-bound state with `xdist_group`** (`conftest.py::pytest_collection_modifyitems`). Under `--dist worksteal`, a stolen test item from a `django.test.TestCase` subclass with class-scoped state (`setUpTestData` overridden or `fixtures` set) would re-trigger `setUpClass`/`setUpTestData` on whichever worker picked it up — defeating the once-per-class setup `TestCase` is designed around. The new logic pins those classes to a per-class `xdist_group=<class qualname>` so worksteal still respects the binding where it matters, while `TransactionTestCase` subclasses (which rebuild per-test regardless) are deliberately *not* pinned and remain free to redistribute.
