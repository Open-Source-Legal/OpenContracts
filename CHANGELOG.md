...
- **Mypy baseline drained: `opencontractserver.llms.tools.*` graduated** (Issue #1447, follow-up to #1331): Removed the six `[mypy-opencontractserver.llms.tools.*]` `ignore_errors` blocks from `mypy.ini` (`core_tools`, `core_tools.*`, `image_tools`, `moderation_tools`, `pydantic_ai_tools`, `tool_factory`, `tool_registry`) and pruned the corresponding entries from `docs/typing/mypy_baseline.txt`. Return-annotation coverage of the package was already at 90.7% (137/151) so the work was almost entirely error-fixing rather than annotating.
  - `opencontractserver/llms/tools/core_tools/`: removed dead `# type: ignore` comments on `channels.db` imports and `FieldFile.read()` / `.open()` calls; widened `search_document_notes` / `asearch_document_notes` return types from `list[dict[str, str | int]]` to `list[dict[str, str | int | None]]` to match the actual `note.created.isoformat() if note.created else None` value; added explicit `raise AssertionError("unreachable")` after the entity-type elif chains in `create_markdown_link` / `acreate_markdown_link` so mypy can prove the functions always return.
...
- **Mypy: `opencontractserver.extracts` return-annotation coverage** (Issue #1447): The `extracts` package was already absent from the mypy baseline (no `ignore_errors` block). Added `-> None` annotations to remaining unannotated functions: `ExtractsConfig.ready` (`apps.py`), `Column.clean` / `Datacell.clean` / `Datacell._validate_manual_entry` / `Datacell._validate_numeric_range` / `Datacell._validate_string_constraints` / `Datacell.save` (`models.py`). Signal handlers in `signals.py` had already received fully-typed annotations via #1413 (kept as-is on merge).
