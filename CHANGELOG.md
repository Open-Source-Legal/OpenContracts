...
- **Frontend unit coverage now uses Istanbul instead of v8** (`frontend/vite.config.ts:210-234`, `frontend/package.json`, `frontend/yarn.lock`): Swapped `@vitest/coverage-v8` for `@vitest/coverage-istanbul` and changed `coverage.provider` from `"v8"` to `"istanbul"`. v8 coverage emits a `DA:` lcov record for roughly every physically executable source line (imports, declarations, single-line statements at the end of a block, etc.) because that is what V8's native coverage API reports for the engine's own profiling/debugging purposes. Istanbul emits `DA:` records only for instrumented statements. The Playwright component and E2E suites already use Istanbul (via `vite-plugin-istanbul` + `nyc`), so as long as unit tests used v8 the three per-suite lcovs measured different things — v8 reported ~183 lines/file vs Istanbul's ~61 statements/file, a ~3× denominator gap on the same files under `frontend/src/`. When Codecov aggregated the three uploads under the merged `frontend` flag, the v8 upload's larger denominator dominated, so the badge landed at ~44% even though the Istanbul-based `frontend-component` flag alone was ~61% on the same code paths. Standardising on Istanbul across all three suites makes the merged flag apples-to-apples and moves the badge into statement-coverage territory comparable to other JS/TS projects. Runtime trade-off: Istanbul instruments source ahead of execution, so unit tests take slightly longer under coverage; in exchange, all three frontend flags (and the merged one) are now directly comparable.
