...
- **Pre-fill defaults in the new-corpus modal**:
  - **Default LabelSet seeded at install** (`opencontractserver/annotations/label_set_seeds.py`, migration `annotations/0069_labelset_is_default_and_seed_default.py`): a public `LabelSet` titled "Default Labels" is now created on install (owned by the first superuser) with a small starter palette — Important, Question, Definition, Reference. New `LabelSet.is_default` BooleanField with a partial unique constraint ensures at most one labelset is flagged as the default. Idempotent management command `python manage.py seed_default_labelset` (`opencontractserver/annotations/management/commands/seed_default_labelset.py`) re-runs the seed safely after the first superuser is created.
