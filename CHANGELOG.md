...
- **Structured extraction no longer masks LLM infrastructure errors as "information not present in document"** (Issue #1351, `opencontractserver/llms/exceptions.py`, `opencontractserver/llms/agents/pydantic_ai_agents.py:1291-1410`, `opencontractserver/llms/agents/core_agents.py:599-652`, `opencontractserver/tasks/data_extract_tasks.py:290-400`): Previously, any exception from the structured-response call in `PydanticAIDocumentAgent._structured_response_raw` was swallowed and converted into a `None` return, so `doc_extract_query_task` reported every OpenAI 429 / `insufficient_quota`, Anthropic `credit_balance_too_low`, 401 auth error, and 5xx failure with the same warning: `"Extraction returned None… This likely means the requested information is not present in the document."` Operators debugging poor extraction F1 had no way to tell a broken pipeline from an exhausted API key.
  - New `StructuredResponseError` hierarchy in `opencontractserver/llms/exceptions.py`: `RateLimitError`, `BillingError`, `AuthenticationError`, `ServerError`, and a generic `ModelError` (non-infra). Each carries a stable `error_code` plus the original `status_code`, `body`, and wrapped exception. A `classify_model_exception()` helper inspects status codes and body markers (`insufficient_quota`, `credit_balance_too_low`, `rate_limit_error`, …) to pick the right subclass — billing markers win over 429-by-status so OpenAI's quota exhaustion is not misreported as rate-limiting.
  - `_structured_response_raw` now raises a classified `StructuredResponseError` instead of returning `None` on exceptions. `ToolConfirmationRequired` still propagates unchanged for the approval-gate flow.
  - `CoreAgentBase.structured_response` re-raises `StructuredResponseError` so callers can distinguish infra failure from a legitimate `None` extraction; unclassifiable exceptions still fall back to the historical `None` return for backwards-compatibility.
  - `doc_extract_query_task` catches `StructuredResponseError` separately and writes the error code into `Datacell.stacktrace` as `[rate_limit_error] …`, `[billing_error] …`, `[authentication_error] …`, `[server_error] …`, or `[model_error] …`. The misleading "information not present" log line is gone for these cases; the `answer_was_none` path now uses its own explicit marker.
  - Added `opencontractserver/tests/test_structured_response_errors.py` covering the classifier, propagation through `structured_response`, and the acceptance test that a rate-limit (and billing/auth/server) failure reaching `doc_extract_query_task` is no longer silent.
