...
- **Migrate active codebase off PAWLs v1; v2 is now canonical end-to-end.** v1 is accepted only at well-defined I/O boundaries (import wire format, plasmapdf hand-off, export wire format). All in-memory consumers — backend and frontend — operate on v2 dicts or v2 typed views.
  - **Backend boundary module** (`opencontractserver/utils/pawls_io.py`, new): single load entry point. `to_canonical_v2(raw)` accepts v1 list or v2 dict and always returns v2 (idempotent; raises on garbage). `load_canonical_v2(file)` reads from `FieldFile`, file-like, or path. `iter_pages` yields `PageView`/`TokenView` zero-copy attribute accessors (`token.x`, `token.isImage`, …). `to_v1_pages(canonical_v2)` is the boundary-only v2→v1 adaptor reserved for plasmapdf and export wire.
  - **Boundary-strict semantics**: pages exceeding `COMPACT_PAWLS_MAX_TOKENS_PER_PAGE` (100,000) previously fell back to v1 silently on disk. The new `to_canonical_v2` raises `ValueError` instead — refusing to leak v1 past the load boundary.
  - **Five import sites** (`opencontractserver/{utils/importing.py, utils/import_v2.py, tasks/import_tasks.py, worker_uploads/tasks.py, pipeline/base/parser.py}`) routed through `to_canonical_v2`.
  - **18 internal v1 readers migrated** to `load_canonical_v2` + `iter_pages`/v2 dict walking: `shared/decorators.py` (×2), `tasks/{doc_tasks,data_extract_tasks}.py`, `annotations/{utils.py, models.py}`, the `populate_content_modalities` management command, `llms/tools/image_tools.py`, `llms/tools/core_tools/{search,document_indexing,annotations}.py`, `utils/{pdf_token_extraction,multimodal_embeddings,extraction_grounding,export_v2,etl,importing}.py`. Tests updated alongside.
  - **Seven boundary call sites** for plasmapdf `build_translation_layer` and export wire-format payload assembly now go through `pawls_io.to_v1_pages` (was `compact_pawls.expand_pawls_pages`) — explicit boundary intent at every site.
  - **Type system**: new `CompactPawlsV2Type`, `CompactPawlsPageType`, `CompactPawlsTokenType`, `CompactImageMetaType` aliases in `types/dicts.py`. Existing `PawlsPagePythonType`/`PawlsTokenPythonType` annotated as v1-wire-only.
  - **Frontend**: `frontend/src/utils/compactPawls.ts` rewritten — `decodeV2Pawls(json)` accepts v1 or v2 wire input but always returns v2-canonical typed objects (`CompactPage[]`). `expandPawlsPages` and the v1 type aliases (`Token`, `PageTokens`, `Page`) deleted. Hot-path migrations in `frontend/src/{utils/transform.tsx, components/annotator/types/pdf.ts, components/annotator/api/{rest,cachedRest}.ts}` (`page.page.width` → `page.width`; `is_image` → `isImage`; `image_path` → `imageMeta?.p`; etc.). The other recon-listed files needed no changes — token field-name access (`x`/`y`/`width`/`height`/`text`) is unchanged on `CompactToken`.
  - **IndexedDB cache invalidation** (`frontend/src/services/documentCacheManager.ts`): DB version bumped 2 → 3 with an upgrade hook that clears stores so stale v1-shape pawls entries are re-fetched and decoded through the new pipeline.
  - **Tests**: new `opencontractserver/tests/test_pawls_io.py` (17 tests) covering boundary semantics, view ergonomics, and v1↔v2 round-trip equivalence. Frontend `compactPawls`, `transform.normalizeTokens`, and `pdf` tests rewritten for the v2 wire fixtures + new in-memory shape.
  - **Architecture doc** (`docs/architecture/pawls-format.md`): "Accessor Layer" section replaced with "Boundary Layer" — documents the four allowed v1 boundaries, the v2 canonical contract, and the strict-fallback semantics.
  - **On-disk format note for image tokens with no v1 metadata fields** (`opencontractserver/utils/compact_pawls.py`, `_compact_token`): v1 → v2 conversion now appends an empty ``{}`` 6th element for image tokens even when no v1 image-metadata fields are present. The previous `if img_meta:` guard would omit the 6th element entirely in that case, which would prevent `TokenView.is_image` from detecting the token as an image after a load. Newly written compact files therefore differ from files written before this PR for image tokens that lack metadata; the round-trip is preserved (`is_image=True` survives) but byte-level equality with pre-PR encodings will not hold for that specific subset. Locked down by `RoundTripSemanticsTests.test_image_token_no_metadata_round_trip` in `opencontractserver/tests/test_pawls_io.py`.
