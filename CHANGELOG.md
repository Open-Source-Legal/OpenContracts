...
- **Mypy baseline drained: `opencontractserver.llms.tools.*` graduated** (Issue #1447, follow-up to #1331): Removed the six `[mypy-opencontractserver.llms.tools.*]` `ignore_errors` blocks from `mypy.ini` (`core_tools`, `image_tools`, `moderation_tools`, `pydantic_ai_tools`, `tool_factory`, `tool_registry`) and pruned the corresponding 25 entries from `docs/typing/mypy_baseline.txt` (7208 → 7183). Return-annotation coverage of the package was already at 90.7% (137/151) so the work was almost entirely error-fixing rather than annotating.
  - `opencontractserver/llms/tools/core_tools.py`: removed seven dead `# type: ignore` comments on `channels.db` imports and `FieldFile.read()` / `.open()` calls (lines 413/416/418/420/491/550/630), widened `search_document_notes` / `asearch_document_notes` return types from `list[dict[str, str | int]]` to `list[dict[str, str | int | None]]` to match the actual `note.created.isoformat() if note.created else None` value (lines 1356, 1423), and added explicit `raise AssertionError("unreachable")` after the entity-type elif chains in `create_markdown_link` / `acreate_markdown_link` so mypy can prove the functions always return (lines 2530, 2693).
  - `opencontractserver/llms/tools/image_tools.py`: dropped the redundant `images: list[ImageData] = []` re-annotation (the variable was already typed at the fast-path branch) and scoped a `# type: ignore[attr-defined]` on `Document.objects.filter(...).visible_to_user(user)` since django-stubs returns the generic `QuerySet[Document, Document]` rather than the project's custom `PermissionQuerySet`.
  - `opencontractserver/llms/tools/moderation_tools.py`: scoped `# type: ignore[attr-defined]` on `message._skip_signals = True` (the `_skip_signals` attribute is a runtime convention used to suppress signal handlers, not a model field).
  - `opencontractserver/llms/tools/pydantic_ai_tools.py`: changed `vector_store: CoreAnnotationVectorStore = Field(default=None, ...)` to `Optional[CoreAnnotationVectorStore]` (the field already accepts `None`), and scoped `# type: ignore[attr-defined]` on the four `async_wrapper.<attr> = ...` metadata assignments (`__signature__`, `_pydantic_ai_wrapper`, `core_tool`, `requires_approval`) — these are intentional dynamic attributes used for pydantic-ai introspection and approval-gate wiring.
  - `opencontractserver/llms/tools/tool_factory.py`: hoisted `param_descriptions = self.metadata.parameter_descriptions or {}` out of the parameter loop in `parameters` so the `.get()` call no longer hits the `Optional[dict[str, str]]`.
  - `opencontractserver/llms/tools/tool_registry.py`: tightened `tool_class: type | None` to `tool_class: type[BaseTool] | None`, added a `TYPE_CHECKING` import block for `BaseTool` and `CoreTool`, and dropped the now-unnecessary `# noqa: F821` on the `to_core_tool` return annotation.
- **Mypy: `opencontractserver.extracts` return-annotation coverage 47.1% → 94.1%** (Issue #1447): The `extracts` package was already absent from the mypy baseline (no `ignore_errors` block), but its return-annotation coverage was below the issue's 70% bar. Added `-> None` annotations to the eight remaining unannotated functions: `ExtractsConfig.ready` (`apps.py`), `handle_datacell_save` / `handle_datacell_delete` (`signals.py`), `Column.clean` / `Datacell.clean` / `Datacell._validate_manual_entry` / `Datacell._validate_numeric_range` / `Datacell._validate_string_constraints` / `Datacell.save` (`models.py`).
