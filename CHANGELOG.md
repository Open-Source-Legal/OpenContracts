...
- **LLM configuration system — Phase 5 (frontend admin + per-column picker)** — completes the user-facing surface. Operators can now register / edit / archive LLMs and configure provider API keys via `/admin/llms`; users with column-edit permission can pin an LLM per column from the column edit modal.
  - **`frontend/src/components/admin/llm_configs/`** — new admin page registered as `/admin/llms` in `App.tsx`. Two tabs:
    - **Providers** — lists every code-side provider class returned by `llmProviders`. Each card renders the provider's `settingsSchema` as a form (secrets are `<input type="password">` placeholders never pre-filled with the existing value; non-secret fields like `base_url` / `organization_id` pre-populate from `currentValue`). Save merges into `LLMSettings.encrypted_secrets` + `provider_settings`; Clear deletes the bucket. Live status pill: Configured / Has secret · invalid / Not configured.
    - **Models** — lists every `RegisteredLLM` with `onlySelectable: false` so superusers can see superseded / archived rows. Default-extract-LLM banner at the top. Per-row controls: Set as default extract LLM · Disable / Enable · Archive (one-way; refused while default). Lineage edits create new versions automatically (backend-side immutable history). Register-new-LLM form picks a provider from the registry, autocompletes from `provider.defaultModels`.
    - All actions: success/failure via `react-toastify`; refetch after every mutation. Lean styled-components — does NOT replicate the full SystemSettings styling subsystem (a follow-up adopt — `LLMSettingSchemaType` was deliberately shaped identical to `ComponentSettingSchemaType` so the existing `AdvancedSettingsPanel` form-rendering code can be reused).
    - `frontend/src/components/admin/llm_configs/graphql.ts` — local GraphQL ops + TypeScript stubs for every Phase-3 query / mutation. Adding to `graphql-api.ts` is a follow-up cleanup.
  - **`frontend/src/components/extracts/ColumnLLMPicker.tsx`** — reusable `<select>` populated by `registeredLlms(onlySelectable: true)`. Empty value = "use system default". Resolvable rows are selectable; non-resolvable rows render `disabled` with their `unavailableReason` as the tooltip — operators see *why* an option is greyed out rather than guessing.
  - **`frontend/src/components/widgets/modals/CreateColumnModal.tsx`** — wires `ColumnLLMPicker` into the column create / edit modal next to the existing Extract Task dropdown. Pre-populates from `formData.preferredLlm.id` when editing; passes `preferredLlmId` (or `"0"` to clear) on submit.
  - **`frontend/src/graphql/mutations.ts`** — `RequestCreateColumnInputType` and `RequestUpdateColumnInputType` gain optional `preferredLlmId?: string`. The `REQUEST_CREATE_COLUMN` and `REQUEST_UPDATE_COLUMN` mutations accept the new variable and request `preferredLlm { id displayName modelId isResolvable }` in the response so picker pre-population just works on the next render.
  - **Verification status**: `tsc --noEmit` exits clean for all touched files (the only flagged error is a pre-existing `tsconfig.json` deprecation warning unrelated to this change). Prettier auto-formatted. UI behavior NOT verified in a browser — admin page styling is functional but not visually polished and may need design pass against the rest of `/admin/*`. The frontend tests for these new components are deferred (they need the full Apollo MockedProvider + Jotai wrapper pattern from `DocumentKnowledgeBaseTestWrapper`, which is significant scaffolding for a v1 admin tool).
- **LLM configuration system — Phase 4 (per-column LLM override + per-cell forensic trace)** — the user-facing payoff of the LLM config system. Operators can now pin a specific `RegisteredLLM` to a `Column`; the extract task uses it instead of `LLMSettings.default_extract_llm`. Every cell stores a `Datacell.executed_llm` FK to the exact (immutable) lineage version that ran it.
  - **`opencontractserver/extracts/models.py`** — adds `Column.preferred_llm` and `Datacell.executed_llm`. Both are nullable FKs to `llm_configs.RegisteredLLM` with `on_delete=PROTECT`. PROTECT aligns with the immutable-history contract on `RegisteredLLM` (no delete mutation exists; PROTECT is defence in depth + the right semantic signal that historical Columns / Datacells must always trace to a real row).
  - **`opencontractserver/extracts/migrations/0029_add_llm_fks.py`** — additive schema migration. Depends on `llm_configs.0001_initial`. No data migration needed; both fields default to NULL.
  - **`opencontractserver/llms/resolution.py:resolve_extract_llm(column=None)`** — gains optional `column` kwarg. Two-stage lookup: (1) if `column.preferred_llm_id` is set, resolve it and **never silently fall back** if it's broken (raises `failure_mode=llm_unavailable` so the explicit pick fails loudly); (2) otherwise fall back to `LLMSettings.default_extract_llm`. Pre-Phase-4 callers that don't pass `column=` keep working unchanged (`column=None` short-circuits past the override path).
  - **`opencontractserver/tasks/data_extract_tasks.py:doc_extract_query_task`**:
    - Passes the cell's `column` to `resolve_extract_llm(column=column)` so the per-column override is honored.
    - Captures `resolved.registered_llm_id` into a local `executed_llm_id` variable.
    - Threads `executed_llm_id` through `sync_mark_completed` and `sync_mark_failed` (both gained an `executed_llm_id` keyword) so every cell that's been executed (success or structured failure) stores the exact lineage version that ran it. NULL on legacy `DEFAULT_EXTRACT_MODEL` fallback paths.
    - The catch-all `except` block reads `executed_llm_id` via `locals().get()` so it can record the row even if the failure happened before normal cleanup (e.g. mid-resolution).
  - **GraphQL — `config/graphql/extract_types.py`**:
    - `ColumnType.preferred_llm: RegisteredLLMType` — resolves the configured LLM (null if none set).
    - `ColumnType.available_llms: [RegisteredLLMType!]!` — picker contents (`RegisteredLLM.objects.selectable()` — head AND enabled AND not archived). Authenticated users see the same list; the picking permission boundary is enforced upstream by the column-edit-permission check, not here.
    - `DatacellType.executed_llm: RegisteredLLMType` — the row that produced this cell's data. Even when a row is later edited (which creates a new lineage version), this FK still points at the original — true forensic immutability.
  - **GraphQL — `config/graphql/extract_mutations.py`**:
    - `CreateColumn` and `UpdateColumnMutation` accept optional `preferred_llm_id: ID`. Pass an int PK to set, `"0"` / `"null"` / `"None"` (or omit on Update) to leave unchanged on Update / null on Create. Validates the target is a head row, enabled, and not archived — refuses to point a Column at an archived/disabled/non-head row at the mutation layer (defense in depth alongside the resolver's own checks).
  - **Tests — `opencontractserver/tests/test_phase4_per_column_llm.py`**: resolver dispatch (column wins over default, no-column falls back, broken column raises `llm_unavailable`, no-default-no-column raises `llm_not_configured`, no-column-arg-at-all preserves pre-Phase-4 contract), GraphQL `ColumnType.preferredLlm` + `availableLlms` (selectable filter excludes archived/disabled/non-head), `DatacellType.executedLlm` (resolves and null), `createColumn` / `updateColumnMutation` accept `preferred_llm_id` + reject every invalid target type.
- **LLM configuration system — Phase 3 (GraphQL surface)** — superuser-only admin API for the LLM config system. Mirrors `pipeline_settings_mutations.py` style: `RateLimits.WRITE_LIGHT` (30 req/min) on every mutation, generic auth-failure messages so non-superusers can't enumerate via timing or wording, secrets never leave the server.
  - **Types — `config/graphql/llm_types.py`**:
    - `LLMSettingSchemaType` — one provider config field. Identical shape to `ComponentSettingSchemaType` so the existing admin UI's form-rendering code applies unchanged.
    - `LLMProviderType` — code-side provider class metadata (`name`, `class_path`, `title`, `pydantic_ai_prefix`, `default_models`, `supports_structured_output`, `supports_tools`), full `settings_schema`, `has_secrets` (any key in the provider's encrypted bucket), `has_valid_secrets` (resolver's "non-empty `api_key`" definition).
    - `RegisteredLLMType` — admin-curated row. Resolves `is_resolvable` and `unavailable_reason` via the `RegisteredLLM` model methods (which delegate to the resolver — single-source-of-truth). Exposes `is_head` (lineage CTE), `is_default_for_extracts`, `pydantic_ai_model_string`, `previous_version_id`, capability overrides, audit fields.
    - `LLMSettingsType` — singleton view: `provider_settings`, `default_extract_llm`, `providers_with_secrets` list, audit fields.
  - **Queries — `config/graphql/llm_queries.py`** registered as `LLMQueryMixin` in `config/graphql/queries.py`:
    - `llmProviders` — superusers see full schema + `has_secrets`/`has_valid_secrets`; non-superusers see public metadata only (no schema, both flags forced false).
    - `registeredLlms(onlySelectable: Boolean = true)` — defaults to head-and-enabled-and-not-archived. Non-superusers can never opt out of the filter; superusers can pass `false` to see the full lineage / archived set.
    - `llmSettings` — superusers get the full payload; non-superusers see only `default_extract_llm` (so the chat UI can label "Default LLM: gpt-4o-mini" without exposing config).
  - **Mutations — `config/graphql/llm_mutations.py`** registered in `config/graphql/mutations.py`:
    - `RegisterLLMMutation` — create a new lineage root (caller-supplied `provider_class_path`, `model_id`, `display_name`, optional capability overrides). Validates provider class is registered, model_id non-empty, display_name non-empty, length caps mirrored from `pipeline_settings_mutations.MAX_*`.
    - `UpdateRegisteredLLMMutation` — *creates a new lineage version* with the supplied changes (immutable history); only fields with non-null arguments change, all others copy forward. Refuses non-head rows ("update the head version instead") and archived rows ("register a new lineage instead"). When the prior row was the default extract LLM, the new head is automatically promoted to default — operators don't have to re-pick after every edit.
    - `ArchiveRegisteredLLMMutation` — one-way archive via a new lineage version with `is_archived=True`. Refuses if the row is currently set as `LLMSettings.default_extract_llm` (operators must promote a new default first via `setDefaultExtractLlm`).
    - `UpdateLLMProviderSecretsMutation` — Fernet-encrypted secrets (api_key, …) plus optional non-secret kwargs (`base_url`, `organization_id`, …) into `LLMSettings.provider_settings[class_path]`. `merge=True` (default) merges; `merge=False` replaces both buckets atomically.
    - `DeleteLLMProviderSecretsMutation` — clear all secrets + non-secret settings for a provider class path. Allowed even for de-registered providers so operators can clean up after a code removal.
    - `SetDefaultExtractLLMMutation` — point `LLMSettings.default_extract_llm` at a row (`id` required) or clear it (`id=null` reverts to legacy `DEFAULT_EXTRACT_MODEL` env-var fallback). Refuses archived/disabled/non-head targets.
    - **No `DeleteRegisteredLLMMutation`. Ever.** Rows are immutable history per the design contract.
  - **Tests — `opencontractserver/tests/test_llm_graphql.py`** (~620 lines, 12 test classes) cover: query visibility (superuser sees full schema + secrets, regular user sees only public metadata + default), `registeredLlms` filtering (head-only, regular user can't opt out, lineage walks correct), every mutation happy path, every mutation refusal (auth failure, validation failure, head-only constraint, archive-while-default refusal, RegisteredLLM-not-found 404, archived/disabled/non-head default refusals), default rolls forward on edit (operators don't lose the resolver after every minor change).
- **LLM configuration system — Phase 2b (chat agent paths wired to resolver)** — completes Phase 2b. Every production agent entry point now picks up admin-configured api_keys via the LLMSettings system. Fresh deploys with no admin config keep using `settings.OPENAI_*` env-var defaults (legacy path preserved); admin-configured deploys with a broken default fail loudly with `failure_mode=llm_unavailable`.
  - **`opencontractserver/llms/resolution.py`** — `resolve_default_llm()` added as a semantic-name alias for `resolve_extract_llm()`. Both return the same row today; the alias gives chat / corpus-action / websocket call sites a stable name so a future migration to a separate `LLMSettings.default_chat_llm` field only needs to update one function.
  - **`opencontractserver/llms/agents/agent_factory.py`** — new `_maybe_resolve_default_llm(model)` helper used by both `UnifiedAgentFactory.create_document_agent` and `create_corpus_agent`. When the caller didn't pin a `model`, the helper calls `resolve_default_llm()` via `sync_to_async`. On success it returns `(model_string, pydantic_ai_model_object, api_key, base_url)` so the factory can build an `AgentConfig` whose `model_name` is the string identifier (token-window helpers stay happy) AND whose new `pydantic_ai_model` field carries the pre-built pydantic-ai Model with the admin's api_key baked in. Failure-mode handling: `llm_not_configured` → return inputs unchanged (factories fall back to `settings.OPENAI_*`); `llm_unavailable` → propagate so the chat session aborts.
  - **`opencontractserver/llms/agents/core_agents.py`** — `AgentConfig` grows `pydantic_ai_model: Optional[Any] = None`. `get_default_config()` now coerces a non-string `model_name` to its string identifier (extracts `.model_name` attr, falls back to `str()`) so legacy helpers like `get_context_window_for_model` and `is_anthropic_model` continue to work whether callers pass strings or pydantic-ai Model objects up the chain. **This is also the fix for a regression introduced in the previous Phase 2b-extracts commit**: `data_extract_tasks` was passing a Model object as `model=` to `for_document`, propagating through to `config.model_name`, and breaking `get_context_window_for_model`.
  - **`opencontractserver/llms/agents/pydantic_ai_agents.py`** — both `PydanticAIAgent(...)` construction sites (document agent at ~line 2516, corpus agent at ~line 2914) now do `model_arg = config.pydantic_ai_model or config.model_name` so the resolver-built Model wins when present, with a clean fall-through to the string identifier for legacy / no-config deploys. `_structured_response_raw` extends the same precedence rule: explicit `model=` override wins, then `config.pydantic_ai_model`, then `config.model_name`. The Anthropic-prefix check (Issue #1381) extracts a string identifier via `getattr(m, "model_name", "")` regardless of which form is in use.
  - **`opencontractserver/llms/api.py`** — new `_model_arg_to_string(model)` helper coerces a `ModelArg` to a plain string; the five sites that call into `UnifiedAgentFactory.create_*_agent` (i.e. through `for_document` / `for_corpus`) now pass `model=_model_arg_to_string(model)` so the base agent's `config.model_name` is always a string. The three sites that call `agent.structured_response(model=…)` still pass the original `ModelArg` (str or Model) — that's the structured-run override path where Model objects are explicitly supported.
  - **Tests** — `opencontractserver/tests/test_llm_chat_wiring.py` covers `_maybe_resolve_default_llm` (caller-pinned passes through, no-default returns inputs unchanged, resolved tuple on success, `llm_unavailable` propagates), `resolve_default_llm` aliasing (same failure mode as `resolve_extract_llm` on no-default deploys), `AgentConfig.pydantic_ai_model` field defaults / accepts arbitrary objects, and `get_default_config` coercion (string passes through, object with `model_name` attr is unwrapped, object without falls back to `str()`).
  - **Coverage of audit findings**: every call site flagged in the previous Phase 2b-extracts commit is now resolver-aware:
    - `tasks/agent_tasks.py:183, 195` (corpus action chat) → calls `agents.for_corpus` → `UnifiedAgentFactory.create_corpus_agent` → `_maybe_resolve_default_llm` → `config.pydantic_ai_model` → corpus-agent construction site honors it.
    - `tasks/agent_tasks.py:870` (document corpus actions) → same chain through `create_document_agent`.
    - `tasks/agent_tasks.py:1167` (corpus corpus actions) → same chain through `create_corpus_agent`.
    - `pydantic_ai_agents.py:2779` (nested document agent) → goes through `agents.for_document` → same chain.
    - `config/websocket/consumers/unified_agent_conversation.py:488, 501` (WebSocket chat) → same chain through both factory paths.
- **LLM configuration system — Phase 2b (extract path wired to resolver)** (first commit that *changes runtime behavior* of extracts; chat agent paths still use the legacy env-var path and are scheduled for a follow-up commit).
  - **`opencontractserver/llms/resolution.py`** — `LLMUnavailableError.failure_mode` now distinguishes two outcomes: `"llm_not_configured"` (no admin default set — caller falls back to legacy `DEFAULT_EXTRACT_MODEL` + env-var keys, preserves pre-Phase-2 behavior) vs `"llm_unavailable"` (a default *is* set but isn't currently resolvable — caller should fail loudly so admin misconfiguration is visible). Constants `FAILURE_MODE_NOT_CONFIGURED` / `FAILURE_MODE_UNAVAILABLE` exported for callers.
  - **`ResolvedLLM.to_pydantic_ai_model()`** — new method that materialises an explicit `pydantic_ai.models.*` Model instance with the admin-configured `api_key` / `base_url` / `organization_id` baked in. Lets pydantic-ai use *that* key instead of whatever's in the worker's env vars (`OPENAI_API_KEY` / `ANTHROPIC_API_KEY`). Defensive branch raises `LLMUnavailableError` if the provider was de-registered between resolution and call time.
  - **Provider classes** — `BaseLLMProvider.build_pydantic_ai_model(model_id, *, api_key, base_url=None, organization_id=None)` is now an abstract classmethod (raises `NotImplementedError` on the base). `OpenAIProvider.build_pydantic_ai_model` returns `pydantic_ai.models.openai.OpenAIChatModel(model_id, provider=OpenAIProvider(api_key=…, base_url=…, organization=…))`. `AnthropicProvider.build_pydantic_ai_model` returns `pydantic_ai.models.anthropic.AnthropicModel(model_id, provider=AnthropicProvider(api_key=…, base_url=…))`. Imports are lazy so the base providers package stays loadable in environments that mock pydantic-ai.
  - **`opencontractserver/llms/agents/pydantic_ai_agents.py:_structured_response_raw`** — `model` parameter widened from `Optional[str]` to `Optional[Union[str, object]]`. When a pydantic-ai Model object is passed, it's used as-is for `PydanticAIAgent(model=…)`; the Anthropic-prefix check (Issue #1381 reliability fix) extracts a string identifier via `getattr(m, "model_name", "")` so the temperature-0 nudge fires correctly. The string-only legacy path is unchanged.
  - **`opencontractserver/llms/api.py`** — five `model: Optional[str]` parameters (on `for_document`, `for_corpus`, `get_structured_response_from_document`, `get_structured_response_and_sources_from_document`, `get_structured_response_from_corpus`) widened to `Optional[ModelArg]` where `ModelArg = Union[str, object]`. No behavior change for legacy string callers; lets the new resolver-driven callers pass through pydantic-ai Model objects end-to-end.
  - **`opencontractserver/tasks/data_extract_tasks.py:doc_extract_query_task`** — `extract_model = model_override or DEFAULT_EXTRACT_MODEL` replaced with a three-priority resolver: (1) `model_override` wins (CLI / benchmark sweep — unchanged); (2) `resolve_extract_llm()` produces a Model with admin-configured api_key; (3) on `failure_mode="llm_not_configured"` falls back to `DEFAULT_EXTRACT_MODEL` + env-var keys (legacy behavior). On `failure_mode="llm_unavailable"` the cell is marked failed via `sync_mark_failed` with `failure_mode=llm_unavailable\n<detail>` written to `Datacell.stacktrace` (so operators can `grep failure_mode=` alongside the existing `_classify_none_result` outcomes). `_resolve_extract_temperature` always sees a string identifier so the Anthropic-temperature-0 logic continues to work whichever path produced the model.
  - **Tests** — `opencontractserver/tests/test_llm_resolution.py` extended: `to_pydantic_ai_model` returns the right `OpenAIChatModel` / `AnthropicModel` with `model_name` attribute; defensive branch raises `LLMUnavailableError` on de-registered provider; `resolve_extract_llm` raises `failure_mode=llm_not_configured` on no-default deploys and `failure_mode=llm_unavailable` on default-but-broken deploys.
  - **Audit — chat agent paths NOT yet wired** (this commit covers extracts only). Production call sites that still resolve api_keys via env vars: `opencontractserver/tasks/agent_tasks.py:183, 195` (corpus action chat agents), `opencontractserver/tasks/agent_tasks.py:870` (document corpus actions), `opencontractserver/tasks/agent_tasks.py:1167` (corpus corpus actions), `opencontractserver/llms/agents/pydantic_ai_agents.py:2779` (nested document agent inside corpus agent), `config/websocket/consumers/unified_agent_conversation.py:488, 501` (the WebSocket chat consumer — every `for_document`/`for_corpus` from the frontend chat). Wiring these will be a follow-up commit that either (a) plumbs `api_key` / `base_url` through `for_document`/`for_corpus` → `AgentConfig` → the two `PydanticAIAgent(...)` construction sites at `pydantic_ai_agents.py:2502, 2895`, or (b) makes `get_default_config` itself call the resolver when no `api_key` override is supplied.
- **`seed_default_llm` management command** — operator-facing bootstrap for the LLM config system added in Phase 2a. Resolves the provider class for a `<prefix>:<model_id>` model string against the provider registry, copies the matching API key out of Django settings (defaults to the provider's declared `env_var`, e.g. `OPENAI_API_KEY`) into `LLMSettings.encrypted_secrets`, creates a `RegisteredLLM` row attributed to the first superuser (overridable with `--creator-username`), and points `LLMSettings.default_extract_llm` at it. Idempotent: re-running on an already-configured deploy is a no-op; `--force` registers a new lineage version pointing at the prior default (immutable history preserved). Adopts an existing matching head row instead of duplicating it. `--dry-run` prints intent without writing. `opencontractserver/llm_configs/management/commands/seed_default_llm.py`. Tests in `opencontractserver/tests/test_seed_default_llm.py` cover bootstrap from env, explicit overrides, idempotency, `--force` lineage versioning, head-row adoption, and every error path (missing api_key, malformed model string, unknown provider prefix, no superuser, unknown override username, dry-run write protection).
- **LLM configuration system — Phase 2a (providers + registry + resolver)** (still no behavioral change to extracts; that comes in Phase 2b): the abstraction layer that turns `RegisteredLLM` rows into runtime values pydantic-ai can consume.
  - **`opencontractserver/llms/providers/`** — new package mirroring the `opencontractserver.pipeline.parsers/` shape:
    - `base.py` — `BaseLLMProvider` abstract base. Class-level metadata (`title`, `description`, `pydantic_ai_prefix`, `default_models`, `supports_structured_output`, `supports_tools`) plus a nested `Settings` dataclass slot using the same `PipelineSetting` metadata machinery as pipeline components, so the existing `settings_schema` extractor (`opencontractserver.pipeline.base.settings_schema.get_settings_schema`) applies unchanged. `class_path()` classmethod returns the canonical `module.ClassName` registry key.
    - `openai.py` — `OpenAIProvider` (covers OpenAI, Azure, OpenAI-compatible). `pydantic_ai_prefix="openai"`, `default_models=("gpt-4o", "gpt-4o-mini", "gpt-4-turbo", "gpt-3.5-turbo", "o1", "o1-mini")`. Settings: `api_key` (SECRET, required, env_var=`OPENAI_API_KEY`), `base_url` (optional override for self-hosted endpoints), `organization_id` (optional `OpenAI-Organization` header).
    - `anthropic.py` — `AnthropicProvider`. `pydantic_ai_prefix="anthropic"`, `default_models=("claude-opus-4-7", "claude-sonnet-4-6", "claude-haiku-4-5", "claude-3-5-sonnet-latest", "claude-3-5-haiku-latest")`. Settings: `api_key` (SECRET, required, env_var=`ANTHROPIC_API_KEY`), `base_url` (optional). The Issue #1381 reliability fixes (forced `temperature=0` for structured runs, `output_retries=3`) in `pydantic_ai_agents.py` continue to apply automatically because they key off the `pydantic_ai_prefix` matching `anthropic`.
    - `registry.py` — `LLMProviderRegistry` mirrors `PipelineComponentRegistry._discover_subclasses`: walks `opencontractserver.llms.providers` via `pkgutil.iter_modules`, deduplicates by class identity, skips abstract classes, swallows per-module import errors. Module-level `get_provider_registry()` is wrapped in `functools.lru_cache(maxsize=1)` for O(1) post-discovery lookup; tests use `reset_provider_registry_cache()` to force re-discovery. `get(class_path)`, `all()`, `class_paths()` are the public surface.
  - **`opencontractserver/llms/resolution.py`** — pure resolution layer that bridges `RegisteredLLM` rows to runtime values:
    - `LLMUnavailableError(RuntimeError)` carries a structured `failure_mode` string (defaults to `"llm_unavailable"`) so call sites that record structured failures (notably `data_extract_tasks._classify_none_result`) can persist a coherent label.
    - `ResolvedLLM` (frozen dataclass) — runtime view: `registered_llm_id`, `provider_class_path`, `provider_title`, `pydantic_ai_model_string` (the `"<prefix>:<model_id>"` string passed straight to `PydanticAIAgent(model=…)`), `api_key`, `base_url`, `organization_id`, `context_window`, `supports_structured_output`, `supports_tools`, `max_output_tokens`, `temperature_default`.
    - `is_resolvable(rl, llm_settings=None)` — checks lifecycle flags, then provider-registry membership, then `LLMSettings.has_valid_secrets`. `unavailable_reason(rl, llm_settings=None)` returns the operator-actionable explanation string the column-picker tooltip will render verbatim, or `None` when resolvable.
    - `resolve(rl)` — materialises a `ResolvedLLM` (raises `LLMUnavailableError` if not resolvable). Centralising the `f"{prefix}:{model_id}"` assembly here means callers never construct provider strings themselves.
    - `resolve_extract_llm()` — Phase-2a entry point that walks `LLMSettings.default_extract_llm`. Phase 4 will add a `column` parameter and prefer `column.preferred_llm` when set. Raises `LLMUnavailableError` on no-default deploys so the Phase-2b caller can fall back to `DEFAULT_EXTRACT_MODEL` (preserves pre-Phase-2 behavior).
  - **`opencontractserver/llm_configs/models.py`** — `RegisteredLLM.is_resolvable()` and a new `RegisteredLLM.unavailable_reason()` now delegate to the resolver via lazy local imports, so the contract is single-sourced. `selectable()` (the picker DB query) remains a lifecycle-only filter — provider+secrets resolvability is enforced at runtime by the resolver, never at the DB layer (would otherwise require the DB to know about the in-process provider registry).
  - **Tests**:
    - `opencontractserver/tests/test_llm_resolution.py` — `ProviderRegistryTests` (auto-discovery picks up shipped providers, skips abstract base, returns None on unknown, every provider has a non-empty `pydantic_ai_prefix`); `IsResolvableTests` (happy path + every failure mode: disabled, archived, unknown provider class path, missing api_key, whitespace-only api_key); `ResolveTests` (pydantic-ai string assembly, capability + override fields flow through, optional `base_url`/`organization_id` from `provider_settings`, `LLMUnavailableError` on unresolvable rows); `ResolveExtractLLMTests` (no-default raises, configured default resolves, default-but-unresolvable raises); `RegisteredLLMResolverDelegationTests` (model methods match resolver output).
    - `opencontractserver/tests/test_llm_configs.py` — `test_selectable_excludes_disabled_and_archived` updated to reflect that `is_resolvable()` is no longer lifecycle-only (the placeholder `provider_class_path` used by the manager test is not registered, so a head with no secrets is correctly *not* resolvable now).
  - **Verification status**: AST + black + isort + flake8 all clean locally; full pytest run still pending (no Docker daemon in my sandbox). Run `docker compose -f test.yml run django pytest opencontractserver/tests/test_llm_configs.py opencontractserver/tests/test_llm_resolution.py -n 4 --dist loadscope` before merge.
- **LLM configuration system — Phase 1 (infrastructure)** (no behavioral change to extracts yet): introduces an admin-curated registry mirroring the existing `PipelineSettings` pattern so future phases can let operators configure permitted LLM providers / models / API keys via the frontend, and let extract Columns pick a per-column LLM. Phase 1 lays only the storage and abstraction floor; provider classes, GraphQL surface, frontend UI, and `Column.preferred_llm` ship in subsequent phases.
  - **`opencontractserver/shared/encrypted_secrets.py`** — new `EncryptedSecretsMixin` extracting the Fernet-over-PBKDF2-derived-from-`SECRET_KEY` encryption used by `PipelineSettings`. The mixin exposes `encrypted_secrets` (BinaryField), `_derive_key`, `get_secrets`, `set_secrets`, `update_secrets`, `get_component_secrets`, and `delete_component_secrets`. Tunables (`PIPELINE_SETTINGS_ENCRYPTION_SALT_LENGTH`, `_ITERATIONS`, `_MAX_SECRET_SIZE_BYTES`) keep their existing names so operator-facing settings continue to work. The same SECRET_KEY-rotation contract applies — rotating the key permanently invalidates all stored secrets across both host models.
  - **`opencontractserver/documents/models.py`** — `PipelineSettings` now inherits from `EncryptedSecretsMixin`; the inline `encrypted_secrets` field declaration and the `_derive_key` / `get_secrets` / `set_secrets` / `update_secrets` / `get_component_secrets` / `delete_component_secrets` / `_get_encryption_*` / `_get_max_secret_size` methods were deleted in favor of inheritance. The schema is unchanged (the mixin's `encrypted_secrets` field is byte-identical to the old inline field, so `makemigrations` should produce no migration). All callers (`pipeline_settings_mutations.py`, `pipeline_queries.py`, `pipeline/base/base_component.py`, `documents/checks.py`, `documents/management/commands/migrate_pipeline_settings.py`, `llms/tools/web_search_tools.py`, `llms/tools/base_tool.py`) hit the same method names with positional args, so the rename of the namespace parameter (`component_path` → `namespace`) on the mixin is source-compatible.
  - **`opencontractserver/llm_configs/`** — new Django app (registered in `LOCAL_APPS` in `config/settings/base.py:147`). Houses two models:
    - **`LLMSettings`** (singleton, `pk=1` enforced via `CheckConstraint`, deletion blocked, cached for `LLM_SETTINGS_CACHE_TTL_SECONDS` (default 300s)) — inherits the secrets mixin. Fields: `provider_settings` (JSON, non-secret per-provider kwargs keyed by provider class path), `default_extract_llm` (FK to `RegisteredLLM`, `on_delete=PROTECT`, NULL-falls-back to `constants.extraction.DEFAULT_EXTRACT_MODEL`), `modified` / `modified_by`. Helper API: `get_provider_settings(path)`, `get_full_provider_settings(path)` (merges plaintext with decrypted secrets, secrets win on collision), `has_valid_secrets(path)` (centralised "non-empty `api_key`" check the resolver will use in Phase 2).
    - **`RegisteredLLM`** (immutable version-chain on `BaseOCModel`) — admin-curated row representing one (provider, model) combination. Fields: `provider_class_path`, `model_id`, `display_name`, `is_enabled`, `is_archived`, `previous_version` (self-FK, `on_delete=PROTECT`), `context_window`, `supports_structured_output`, `supports_tools`, `max_output_tokens`, `temperature_default`, `notes`. Edits never mutate; they create a new row with `previous_version` pointing at the prior one, so `Column.preferred_llm` and `Datacell.executed_llm` (Phase 4) can use `on_delete=PROTECT` for full forensic traceability with zero per-cell snapshot cost. Manager `RegisteredLLMManager` exposes `heads()` (NOT EXISTS subquery picks rows no newer version supersedes) and `selectable()` (heads filtered by `is_enabled AND NOT is_archived`). Instance helpers `is_head()` and `is_resolvable()` (Phase 1: lifecycle flags only; Phase 2 will augment with provider-registry membership and `LLMSettings.has_valid_secrets`). Indexes on `(provider_class_path, model_id)` and `(is_archived, is_enabled)`.
    - **Migration `0001_initial.py`** — creates both tables, the singleton check constraint, and seeds an empty `LLMSettings` row (`pk=1`) so callers can unconditionally `LLMSettings.get_instance()` post-deploy. `RegisteredLLM` seeding is deferred to Phase 2 so the seeded `provider_class_path` is guaranteed to resolve against the (yet-to-exist) `LLMProviderRegistry`.
  - **`opencontractserver/tests/test_llm_configs.py`** — `TestCase` coverage for: singleton invariants (`get_instance` idempotency, second-instance creation rejected, deletion forbidden, cache invalidation on save), `EncryptedSecretsMixin` round-trip on the new host (`set_secrets` + `get_secrets`, `update_secrets` merges per-namespace, `delete_component_secrets` removes only the targeted bucket, `has_valid_secrets` rejects whitespace / empty, `get_full_provider_settings` lets secrets override plaintext), `RegisteredLLM` version-chain (`heads()` / `selectable()` filters, `is_head()` / `is_resolvable()`, `previous_version` PROTECT blocks deletion, `default_extract_llm` PROTECT blocks deletion, DB-level singleton CheckConstraint rejects `pk!=1`).
  - **Verification status**: I authored every file in this changeset, ran AST syntax checks on all of them, and cross-checked the migration against the existing `0031_add_pipeline_settings.py` / `0034_update_checkconstraint_check_to_condition.py` style. I was *not* able to run `makemigrations --dry-run` or the test suite from my development sandbox (no Docker daemon available). Before merging, run: `docker compose -f local.yml run --rm django python manage.py makemigrations --check --dry-run` (must report no changes for `documents`, only the new `llm_configs/0001_initial.py` for `llm_configs`); `docker compose -f local.yml run --rm django python manage.py migrate llm_configs`; `docker compose -f test.yml run django pytest opencontractserver/tests/test_pipeline_settings.py opencontractserver/tests/test_llm_configs.py -n 4 --dist loadscope`.
