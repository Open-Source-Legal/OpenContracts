...
- **Decluttered floating layout around the chat tray input** (`frontend/src/components/knowledge_base/document/FloatingDocumentControls.tsx`, `frontend/src/components/knowledge_base/document/DocumentKnowledgeBase.tsx:2148`). When the right-hand sliding panel is open in chat mode, the document-tool FABs (extracts, analyses, create-analysis) are now hidden so they no longer crowd the panel boundary next to the chat input and sidebar tabs. The panel-width control remains visible since it directly affects the open panel. Closing the chat tray (or switching the sidebar to a non-chat view) restores the FAB stack. Implemented via a new `hideDocumentTools` prop on `FloatingDocumentControls`, applied to both the desktop FAB column and the mobile speed-dial entries.
