...
- **Native PDF link annotations are now clickable in the viewer.** Previously, hyperlinks embedded in PDFs (external URLs surfaced via PDF.js's annotation API) were painted into the page canvas as pixels only — never reachable to mouse events, because the custom `SelectionLayer` (`frontend/src/components/annotator/renderers/pdf/SelectionLayer.tsx`, full-page overlay at z-index 1) intercepted every click before it could reach a link. New `NativeLinkLayer` component (`frontend/src/components/annotator/renderers/pdf/NativeLinkLayer.tsx`) calls `page.getAnnotations()`, filters to `subtype === "Link"` entries with a safe URL (`http:`, `https:`, `mailto:` only — `javascript:` / `file:` rejected), and renders a transparent `<a target="_blank" rel="noopener noreferrer">` over each link's `viewport.convertToViewportRectangle`-projected bounds. Mounted inside `CanvasWrapper` in `PDFPage.tsx` at z-index 2 (above SelectionLayer's 1, below the SelectionActionMenu portal at `Z_INDEX.CONTEXT_MENU`); the overlay container uses `pointer-events: none` with `pointer-events: auto` on each anchor, so empty space falls through to the selection logic while link rectangles capture clicks. Hover surfaces a subtle highlight + tooltip; right-click "open in new tab" and keyboard focus work because the elements are real anchors. Custom OC_URL annotations and overlapping user annotations still win at the same z-index by virtue of being rendered later in the DOM. Internal `dest`-based PDF links (page-to-page navigation) are not yet wired — only external URLs. Coverage: `frontend/src/components/annotator/renderers/pdf/__tests__/NativeLinkLayer.test.tsx` (5 cases: safe-link rendering, unsafe-protocol filtering, non-Link/no-URL skip, empty state, viewport scaling).
