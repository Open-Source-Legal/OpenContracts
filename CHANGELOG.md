...
- **Corpus view layout — content not filling `#main-corpus-content-area`, awkward whitespace below content, and controls offscreen at viewport bottom** (`frontend/src/views/Corpuses.tsx`, `frontend/src/views/Corpuses.styles.ts`, `frontend/src/components/layout/CardLayout.tsx`). Three layered defects broke the flex-height chain from `CardLayout` down to the tab content:
  - **Inline tab wrappers missing flex sizing.** Seven non-chat tabs (documents, annotations, analyses, relationships, discussions, settings, badges) wrapped content in `<div style={{ display: "flex", flexDirection: "column" }}>` with no `flex: 1` / `min-height: 0` / `height: 100%`. The wrapper shrink-wrapped to its content (~487px in the reporter's case) instead of filling `MainContentArea`, leaving a large empty band below the content. Extracted shared `TabContentWrapper` / `TabContentBody` styled components in `Corpuses.styles.ts` and replaced all seven duplications in `Corpuses.tsx` so the flex chain reaches the leaf containers (`BrowserContainer`, `CorpusAnnotationCards`, etc.).
  - **`ScrollableSegment` (`frontend/src/components/layout/CardLayout.tsx:60-68`) didn't pass available height down.** It was a flex column container itself but lacked `flex: 1` and `min-height: 0`, so even with `TabContentWrapper`'s `flex: 1` the chain ended early. Added both properties so the segment now fills `CardContainer` and passes height down to `CorpusViewContainer`.
  - **`CorpusViewContainer` (`frontend/src/views/Corpuses.styles.ts:22-33`) used `min-height: calc(100dvh - navbar-height)`** — a workaround for the broken chain that became incorrect when a `SearchBar` row was rendered above it inside `CardLayout` (documents/annotations/extracts/etc. tabs). The min-height didn't subtract the search-bar height, so the bottom of the view overflowed the visible viewport and any controls anchored near the bottom went offscreen. With the chain now intact upstream, replaced the two `min-height: calc(...)` lines with `min-height: 0` and rely on flex inheritance from `AppShell` → `CardContainer` → `ScrollableSegment`. `CleanViewContainer` (non-power-user mode, never rendered under a `SearchBar`) is intentionally left unchanged.
  - The chats tab (`Corpuses.tsx:1258`) intentionally retains its explicit `height: calc(--oc-visible-viewport-height - --oc-navbar-height)` because the input-pinned-to-bottom / messages-scroll-internally layout depends on that exact bound; changing it would risk regression in `CorpusChat`'s scroll behavior.
