...
- **Anonymous users could see public structural annotations but not their attached images** (`opencontractserver/annotations/views.py`, `opencontractserver/llms/tools/image_tools.py`, `opencontractserver/tests/test_annotation_images_api.py`). `AnnotationImagesView` (`/api/annotations/<id>/images/`) was hardcoded to `permission_classes = [IsAuthenticated]`, so the frontend hook (`frontend/src/components/annotator/hooks/useAnnotationImages.tsx`) received 401/403 for anonymous viewers and silently rendered a "No thumbnail" placeholder — even when the annotation itself was returned by the GraphQL feed under the public/anonymous read rules in `docs/permissioning/consolidated_permissioning_guide.md` (line 577: "Annotation: ✅ Document AND Corpus both public"). The image visibility now mirrors what `AnnotationQuerySet.visible_to_user` exposes:
  - `AnnotationImagesView.permission_classes` is now `[AllowAny]`; throttling is preserved by adding `AnnotationImagesAnonThrottle(AnonRateThrottle)` alongside the existing `AnnotationImagesThrottle(UserRateThrottle)` (both share the `annotation_images` scope so the 200/hour rate applies per IP for anonymous users and per user for authenticated users).
  - `get_annotation_images_with_permission` (`opencontractserver/llms/tools/image_tools.py:472`) gained an explicit anonymous gate: `if user.is_anonymous and not annotation.structural: return []`. This closes a gap where the helper would otherwise grant images for non-structural public annotations that anonymous users cannot see via the queryset (the queryset filters anon visibility down to `Q(structural=True) & doc_public & corpus_public_or_null` in `opencontractserver/shared/QuerySets.py:282-296`). The existing document/corpus public-flag checks via `user_has_permission_for_obj` continue to enforce the "public document AND public corpus" half of the rule. IDOR protection is unchanged — same empty 200 response for missing/unauthorized.
  - `test_fetch_images_unauthenticated` was renamed to `test_fetch_images_unauthenticated_private` and updated from "expect 401/403" to "expect 200 with empty array" (IDOR-consistent shape). Added regression tests pinning all three new anonymous paths: `test_fetch_images_anonymous_public_structural` (allowed → 2 images), `test_fetch_images_anonymous_non_structural_blocked` (denied → empty), `test_fetch_images_anonymous_private_corpus_blocked` (denied → empty).
