...
- **Typing: graduated 11 `opencontractserver.utils.*` modules out of the mypy baseline** (chunk 2 of issue #1481, follow-up to #1331/#1447). The following modules are now type-checked end-to-end and their `[mypy-…]` `ignore_errors` blocks have been removed from `mypy.ini`: `export_v2`, `files`, `import_v2`, `importing`, `logging`, `mention_parser`, `multimodal_embeddings`, `packaging`, `pdf_token_extraction`, `sharing`, `storages`. 54 baseline error entries pruned from `docs/typing/mypy_baseline.txt`. Highlights: V2 export builders now emit narrowly typed `TypedDict` payloads (`StructuralAnnotationSetExport`, `CorpusFolderExport`, `OpenContractsRelationshipPythonType`, `DescriptionRevisionExport`); `packaging.unpack_corpus_from_export` discriminates V1 vs V2 input via the `OpenContractCorpusV2Type` overlay; `multimodal_embeddings` propagates `PawlsTokenPythonType` through the image-token pipeline; `import_v2` and `packaging` use `TYPE_CHECKING` `User` aliases so `User = get_user_model()` is no longer used as a runtime annotation; `sharing.make_analysis_public` now guards against analyses with no `analyzed_corpus`; `files.split_pdf_into_images` preserves its `Literal["PNG", "JPEG"]` parameter narrowing across the case-normalisation step. Pre-commit `mypy` continues to run clean on the full project surface (`opencontractserver` + `config`).
