...
- **Corpus document browser shows 0 docs after import while sidebar shows the correct total** (`config/graphql/filters.py:499-533`, `opencontractserver/corpuses/models.py:887-905`, `opencontractserver/documents/versioning.py:187-198`, `opencontractserver/tasks/import_tasks_v2.py:670-682,714-731`, `opencontractserver/utils/import_v2.py:236-256`). A superuser-reported regression where the post-import corpus surfaced the right count in `corpus.documentCount` (53) but `0` in every folder card and the document browser. Three additive defences land here so the symptom can't recur silently:
  - **`DocumentFilter.in_folder` is now corpus-scoped.** The `__root__` branch previously matched `DocumentPath(folder__isnull=True, is_current=True, is_deleted=False)` *globally* and relied entirely on the chained `in_corpus` filter to project back to the current corpus via `id__in`. Any document with paths in two corpora — one in a folder here, one at root elsewhere — would falsely surface in this corpus's root view (and conversely, the relay totalCount disagreed with the actual edges). The filter now reads `in_corpus_with_id` from `self.data` and restricts the inner `DocumentPath` subquery to the same corpus, so the subquery and the outer filter agree. Same composite index (`corpus, is_current, is_deleted`) supports the new shape — no migration.
  - **`Corpus.add_document` and `import_document` reject foreign folders.** Cross-corpus folder assignment was silently wrong: the `DocumentPath` row counted toward `Corpus.document_count` (corpus-filtered) but vanished from the folder browser (folder-filtered, where the folder lives in a different corpus). Both writers now mirror the parent-folder guard already in `CorpusObjsService.create_folder` and raise `ValueError` at the boundary.
  - **Import-side logging for silent folder drift.** `_reconstruct_document_paths` (`tasks/import_tasks_v2.py`) now logs a warning when a document's exported `folder_path` doesn't resolve to any imported folder (doc stays at root), and tightens the `path_by_doc_id` lookup to `is_current=True, is_deleted=False` so a re-import that picks up a superseded historical path can't silently overwrite the wrong row via dict-comprehension last-write-wins. `import_corpus_folders` (`utils/import_v2.py`) warns when a folder's exported `parent_id` was never created (e.g. parent's `create_folder` failed) so the resulting orphan-at-root case is operator-visible instead of an invisible misplacement.
  - Regression tests: `tests/test_document_filter_in_folder.py` (three-doc fixture covering the cross-corpus root leak, the folder hit, and the symmetric corpus B root), plus `TestAddDocumentCrossCorpusFolderGuard` in `tests/test_corpus_folders.py` (positive + negative case for the boundary guard).
