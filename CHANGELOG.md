...
- **Anonymous users could see public structural annotations but not their attached images** (`opencontractserver/annotations/views.py`, `opencontractserver/llms/tools/image_tools.py:472`, `opencontractserver/tests/test_annotation_images_api.py`, `docs/permissioning/consolidated_permissioning_guide.md`). `AnnotationImagesView` (`/api/annotations/<id>/images/`) was hardcoded to `IsAuthenticated`, so the frontend hook silently rendered "No thumbnail" for anonymous viewers even when the annotation was returned by the GraphQL feed under public-read rules. The endpoint now uses `AllowAny` plus a self-contained anonymous gate in `get_annotation_images_with_permission` that mirrors `AnnotationQuerySet.visible_to_user` exactly (structural=True AND document.is_public AND corpus is null OR corpus.is_public). Throttling is split into authenticated (`AnnotationImagesThrottle`) and anonymous (`AnnotationImagesAnonThrottle`) buckets, each gated on the matching user type so anonymous requests no longer consume two cache slots in lockstep. IDOR protection unchanged — empty 200 for missing/unauthorized. Test matrix pins all four anonymous paths (public-structural allowed, non-structural blocked, private-corpus blocked, private-document blocked) plus the corpus=None branch.
