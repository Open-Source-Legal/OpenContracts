...
- **Document corpus-less `ModerationAction` auth gap with a TODO** (`config/graphql/conversation_queries.py:489`, issue #1594). `resolve_moderation_action` falls through to return the action to any authenticated non-superuser when `action.conversation` is `None` or `action.conversation.chat_with_corpus` is `None`. The canonical `Corpus.user_can_moderate(user)` helper is preserved (no DRY regression — see `Corpus.user_can_moderate`, consolidated under issue #1450); the TODO marks the fall-through branch so a follow-up can mirror the list resolver's stricter "must be corpus owner/moderator" filter once the broader fix lands. No functional behavior change in this PR.
