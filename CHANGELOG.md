...
- **Bulk and single document upload no longer crash with Apollo "Payload allocation size overflow"** before the network request fires. The previous transport base64-encoded the entire file (PDF or zip) into a single GraphQL `String` variable; Apollo then `JSON.stringify`'d the request body, which V8 cannot allocate as a contiguous string for large files (~33% inflation pushes a moderately sized zip past the V8 string limit / heap fragmentation ceiling). The frontend now streams uploads as `multipart/form-data` via `fetch` to two new REST endpoints, so the binary bytes never become a JS string. Files affected:
  ...
  - `frontend/src/components/widgets/modals/UploadModal/hooks/uploadHttp.ts` (new) — multipart helpers (`uploadDocumentMultipart`, `uploadZipMultipart`) that attach the JWT bearer token and parse REST error bodies into structured results.
  - `frontend/src/utils/files.ts` — `toBase64` removed (last call site was the upload hook).

### Added

- **Multipart REST upload endpoints under `/api/uploads/`** (`opencontractserver/document_uploads/`, new app):
  - `POST /api/uploads/documents/` — single-document upload (`DocumentUploadView`).
  - `POST /api/uploads/documents-zip/` — bulk-zip upload (`DocumentsZipUploadView`); stages the archive in a `TemporaryFileHandle` and queues `process_documents_zip` exactly as the GraphQL path did.
  - `opencontractserver/document_uploads/services.py` exposes `upload_document_for_user` and `upload_documents_zip_for_user`. Both `UploadDocument` / `UploadDocumentsZip` GraphQL mutations now delegate to these services so the two transports validate identically (visibility + EDIT permission with unified IDOR-safe error message; usage-cap enforcement; folder-in-corpus check; allowed-MIME enforcement; bulk-job owner cache binding).
  - New setting `MAX_DOCUMENT_UPLOAD_SIZE_BYTES` (env-overridable, defaults to `MAX_FILE_UPLOAD_SIZE_BYTES`) caps multipart upload size at the view layer with an explicit `413` response. Set to `0` to disable.
  - Auth: standard DRF auth — JWT bearer (matches GraphQL) or session.
  - Tests: `opencontractserver/tests/test_document_uploads_rest.py` covers both views (auth, owned/inaccessible/read-only corpus IDOR uniformity, folder-in-corpus check, unsupported MIME, oversize 413, usage-cap, custom-meta JSON pass-through, owner-cache binding) plus direct service tests for the bytes/UploadedFile branches. `frontend/src/components/widgets/modals/UploadModal/hooks/__tests__/uploadHttp.test.ts` (8 cases) verifies the request body is `FormData` — not JSON — with correct URL, bearer auth header, blank-string omission, and structured error parsing. `useUploadMutations.test.tsx` (8 cases) covers sequential upload semantics, status-callback ordering, partial-failure resilience, and refetch+`onComplete` exactly-once.
