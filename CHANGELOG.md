...
- **Phase B of permission centralization: migrate ~40 GraphQL mutation call sites + folder service + DocumentQueryOptimizer off the `user_has_permission_for_obj` shim** (`config/graphql/{base,corpus_mutations,document_mutations,extract_mutations,annotation_mutations,document_relationship_mutations,agent_mutations,analysis_mutations,badge_mutations,label_mutations,smart_label_mutations,conversation_mutations}.py`, `opencontractserver/corpuses/folder_service.py`, `opencontractserver/documents/query_optimizer.py`). Issue #1656 — Phase B of the multi-phase permission centralization (Phase A: PR #1663). Every migrated site replaces a compound `user.is_superuser or obj.creator_id == user.id or user_has_permission_for_obj(user, obj, perm)` (or the simpler `user_has_permission_for_obj(user, obj, perm)`) with a single `obj.user_can(user, perm)` call. The per-model `Manager.user_can` overrides shipped in Phase A now encapsulate the superuser / creator / structural / privacy / inheritance rules so callers stop re-implementing them. `DocumentActionsQueryOptimizer._check_document_permission` and `DocumentActionsQueryOptimizer._check_corpus_permission` (the two bespoke ~40-line compound wrappers) are deleted outright; the orphaned `DocumentRelationshipQueryOptimizer._check_corpus_permission` is deleted too. `DocumentRelationshipQueryOptimizer.user_has_permission` now delegates each of its source/target/corpus checks to `obj.user_can(user, perm)`. Net change: **~340 LOC deleted, ~100 LOC added** across 14 files. Field-resolver / agent-tool / task migrations remain out of scope (tracked under Phase E, issue #1659).

  **Behavioural widenings — intended, documented here for auditors:**

  - **Creator status now honoured at every migrated call site.** The legacy shim defaulted `include_group_permissions=False`, but more importantly, callers that wrapped the shim in `user.is_superuser or obj.creator_id == user.id or user_has_permission_for_obj(...)` carried the creator/superuser short-circuit only at *that* call site. Sites that called the shim directly (without the prefix) skipped the creator branch entirely. `obj.user_can(user, perm)` always applies the superuser-then-creator branches before guardian lookup, so a corpus / document / extract / agent creator who previously needed an explicit guardian grant to update their own resource now passes the check on creator status alone. This matches the existing behavior of `visible_to_user` and removes a long-standing asymmetry between filter and check.
  - **Group permissions now consulted at every migrated call site.** The shim's `include_group_permissions=False` legacy default ignored guardian *group* grants and only honoured *user* grants. `obj.user_can(user, perm)` defaults `include_group_permissions=True` (matching `visible_to_user`). A user who has access to a corpus / document only via a guardian group grant can now exercise the matching mutations; previously, those calls denied even though the same user could see the object via `visible_to_user`.

  Both widenings align `user_can` with `visible_to_user` (Phase A's invariant). Tests verified: 145 mutation tests + 549 permission tests + 175 query-optimizer / corpus-action / personal-corpus tests pass (869 total).
