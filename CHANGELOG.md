...
- **Extract & analyzer agent tools — review follow-ups on PR #1675** (`opencontractserver/llms/tools/core_tools/extracts_and_analyzers.py`, `opencontractserver/constants/tools.py`, `opencontractserver/tests/test_extract_analyzer_tools.py`).
  - **Status vocabulary aligned.** `start_analysis` previously returned `EXTRACT_STATUS_QUEUED` (`"queued"`) while `list_recent_analyses` passed the raw model status through (`JobStatus.QUEUED.value` → `"QUEUED"`). An LLM polling the listing right after dispatch saw the status appear to change without any state transition. New `_normalize_analysis_status` lowercases every status going through the listing tool; `start_analysis` now returns `_ANALYSIS_STATUS_QUEUED` (derived from `JobStatus.QUEUED.value`) so the two surfaces agree on a single lowercase vocabulary that matches the extract tools. `EXTRACT_STATUS_QUEUED` is no longer used outside the extract path, closing the semantic leak flagged in review.
  ...
  - **`list_fieldsets` surfaces an `extractable` flag** so the LLM knows up-front that manual-only or empty fieldsets will be rejected at `start_extract`. Also re-ordered the listing by `-created` (not `-modified`) to match `list_recent_extracts` and avoid silent position shifts when a fieldset is edited mid-session.
  ...
  - New tests pin each contract: status-normalisation in `list_recent_analyses`, reserved-key strip + warning emission in `start_analysis`, untouched legitimate payload in `start_analysis`, oversized-schema truncation in `list_analyzers`, and the `extractable` flag in the manual-only-fieldset listing-vs-dispatch invariant.
