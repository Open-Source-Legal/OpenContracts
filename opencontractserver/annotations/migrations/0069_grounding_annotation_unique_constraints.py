...
Both constraints are scoped via ``condition=Q(structural=False)`` so
structural / layout annotations (which can legitimately repeat across
parser runs) remain unaffected.  ``creator`` is included so two users
manually creating identical annotations are not blocked — only a single
user creating the literal same row is, which matches the realistic
race-condition target (the grounding pipeline always uses the datacell
owner's ID).

Existing duplicates are deduped by keeping the lowest-pk row in each
group and re-pointing dependent FKs (datacell.sources M2M) to it before
deleting the rest.  This is the same pattern used in migration 0068.
"""
...
def _dedupe_token_label(apps):
    """Collapse duplicate non-structural TOKEN_LABEL annotations.
    ...
    if total_groups:
        logger.info(
            "Collapsed %s duplicate non-structural TOKEN_LABEL annotation rows "
...
def _dedupe_span_label(apps):
    """Collapse duplicate non-structural SPAN_LABEL annotations.
    ...
    if total_groups:
        logger.info(
            "Collapsed %s duplicate non-structural SPAN_LABEL annotation rows "
            "across %s groups before adding unique constraint.",
            total_collapsed,
            total_groups,
        )
    ...
    operations = [
        ...
        migrations.AddConstraint(
            ...
            constraint=models.UniqueConstraint(
                ...
                condition=Q(structural=False, annotation_type="TOKEN_LABEL"),
        ...
        migrations.AddConstraint(
            ...
            constraint=models.UniqueConstraint(
                ...
                condition=Q(structural=False, annotation_type="SPAN_LABEL"),
