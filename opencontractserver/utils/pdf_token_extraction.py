...
def build_spatial_indices_from_pawls(
    pawls_pages: list[PawlsPagePythonType],
) -> tuple[dict[int, STRtree], dict[int, np.ndarray]]:
    """
    Build per-page STRtree spatial indices from already-parsed PAWLs pages.

    Mirrors the spatial-index construction in :func:`extract_pawls_tokens_from_pdf`
    but operates on existing token data instead of re-parsing a PDF. Useful when
    PAWLs has already been generated by the parser pipeline and we need to query
    which tokens fall within a bounding box (e.g. recalculating annotation token
    references against freshly-parsed tokens).

    Args:
        pawls_pages: List of PAWLs page dicts (v1 expanded format) with
            ``tokens`` arrays containing ``x``, ``y``, ``width``, ``height``.

    Returns:
        Tuple of:
        - spatial_indices: Dict mapping 0-based page index to STRtree.
        - token_indices_by_page: Dict mapping 0-based page index to numpy array
          of token indices (one entry per geometry in the matching STRtree).
    """
    spatial_indices_by_page: dict[int, STRtree] = {}
    token_indices_by_page: dict[int, np.ndarray] = {}
    ...
    for page_idx, page in enumerate(pawls_pages):
        if not isinstance(page, dict):
            continue
        # Prefer the explicit page index when available; fall back to position.
        page_meta = page.get("page") or {}
        page_num_0based = (
            int(page_meta["index"])
            if isinstance(page_meta, dict) and "index" in page_meta
            else page_idx
        )
        ...
        if geometries:
            spatial_indices_by_page[page_num_0based] = STRtree(np.array(geometries))
            token_indices_by_page[page_num_0based] = np.array(indices, dtype=np.intp)
        else:
            token_indices_by_page[page_num_0based] = np.array([], dtype=np.intp)

    return spatial_indices_by_page, token_indices_by_page
