...
class AnnotationManager(PermissionManager.from_queryset(AnnotationQuerySet)):  # type: ignore[misc]
    ...
    def user_can(
        ...
        *,
        ...
        if isinstance(user, AnonymousUser) or not getattr(
            ...
            return self._anonymous_read_via_visible_to_user(user, instance, permission)
    ...
    def _anonymous_read_via_visible_to_user(
        self, user: Any, instance: Any, permission: Any
    ) -> bool:
        """READ via ``visible_to_user(...).exists()``; deny non-READ.

        Shared by the anonymous branch (where the QuerySet's anonymous
        predicate encodes structural+public-doc+public-corpus rules)
        and the ``document_id is None`` branch (where the QuerySet
        covers the structural_set route).
        """
        from opencontractserver.types.enums import PermissionTypes
    ...
    def _check_annotation_privacy_recursion(
        ...
        """Return ``False`` if the source Analysis/Extract denies access.

        When an annotation was generated by an ``Analysis`` or
        ``Extract``, the requested permission must hold on that source
        object (in addition to doc+corpus). Delegates to
        ``Analysis.objects.user_can`` / ``Extract.objects.user_can`` so
        creator status on the source is honored — fixing the legacy
        bug where the recursion used the creator-blind
        ``user_has_permission_for_obj``.

        Skip for the structural-READ case (structural rows are always
        READable when the parent doc is). At this point the caller has
        already denied non-READ structural calls, so ``structural and
        READ`` is the only structural state we can still be in — but
        we keep the explicit ``and permission == READ`` for readability
        rather than relying on the flow-sensitive equivalence.
        """
        from opencontractserver.types.enums import PermissionTypes
