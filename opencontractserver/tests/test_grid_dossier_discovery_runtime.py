...
class _RuntimeRichProvider(BaseAuthoritySourceProvider):
    ...
    supported_prefixes: ClassVar[tuple[str, ...]] = ("puct-project",)
...
def _record(
    *,
    canonical_key: str = "puct-project:58211:item:4",
    rights_status: RightsStatus = RightsStatus.REVIEW_REQUIRED,
    corpus_slug: str = "puct-large-load-proceedings",
    source_url: str = "https://uscode.house.gov/grid-dossier-test.txt",
    content: bytes = b"Authority record test content",
) -> AuthoritySourceRecord:
    return AuthoritySourceRecord(
        canonical_key=canonical_key,
        title="PUCT Project 58211 Item 4",
...
class GridDossierDiscoveryRuntimeTests(TestCase):
    provider_name: ClassVar[str] = "_RuntimeRichProvider"

    def setUp(self):
        self.user = User.objects.create_user(username="grid-runtime", password="p")

    def _frontier(self, *, approved: bool = False) -> AuthorityFrontier:
        candidate_sources: list[dict] = [
            {
                "discovery_provider": "PUCTProjectDiscoveryProvider",
                "url": "https://uscode.house.gov/grid-dossier-test.txt",
                "title": "Discovered PUCT attachment",
                "extra": {
        ...
        if approved:
            approval_fingerprint = (
                AuthorityDiscoveryService._response_approval_fingerprint(
                    ...
                    fetch_key="puct-project:58211:item:4",
                    request=AuthorityRequest(
                        canonical_key="puct-project:58211:item:4",
                        url="https://uscode.house.gov/grid-dossier-test.txt",
                    ),
                    sections=[_record()],
                )
    ...
    def test_mixed_record_rights_are_rejected_even_with_approval(self):
        ...
        provider = _RuntimeRichProvider(
            [
                ...
                _record(
                    canonical_key="puct-project:58211:item:4:attachment:2",
    ...
    def test_changed_response_url_requires_new_approval(self):
        ...
        provider = _RuntimeRichProvider(
            [
                _record(
                    source_url=(
                        "https://uscode.house.gov/grid-dossier-reseeded-test.txt"
                    )
                )
            ]
        )
    ...
    def test_approve_writes_scoped_durable_audit_record(self):
        ...
        frontier = AuthorityFrontier.objects.create(
            canonical_key="puct-project:58211:item:9",
            authority="puct-project",
            provider=self.provider_name,
