...
class TestSidecarImportTask(_SidecarImportTestMixin, TestCase):
    ...
    The new importer does NOT apply annotations inline. Instead it creates the
    document through the normal parser pipeline, persists the producer
    (dumb-anchor) annotations in a ``PendingDocumentAnnotations`` row, and
    dispatches an ``ingest_doc -> remap_pending_annotations -> set_doc_lock_state``
    chain on commit so the remap runs after PAWLs / text exist. These tests
    assert the persisted pending row + its payload; end-to-end re-anchoring
    through the chain is covered by the post-ingest integration tests.
    """
    ...
    def _run_import(self, files: dict[str, bytes], job_id: str) -> dict:
        """Run the importer and capture (but do NOT execute) on_commit chains.

        We capture with ``execute=False`` so the ingest->remap->unlock chain
        is *queued* but never actually run inline — running it would invoke the
        real parser pipeline (Docling REST), which is out of scope for these
        unit tests and unavailable here. We assert the persisted pending row
        and the queued dispatch; full re-anchoring through the chain is covered
        by the post-ingest integration suite. ``self.captured`` holds the
    ...
    def test_single_annotated_document_persists_pending_row(self):
        ...
        # The ingest->remap->unlock chain dispatch was queued on commit.
