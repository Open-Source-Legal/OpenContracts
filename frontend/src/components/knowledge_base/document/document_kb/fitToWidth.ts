 ...
 * pixels (split between the two horizontal edges), then clamped to the
 * shared {@link ZOOM_MIN} / {@link ZOOM_MAX} bounds. Reserving a small
 * margin prevents the page from butting up against the container edges
 * and — more importantly — prevents horizontal overflow that would
 * otherwise force a horizontal scrollbar at the moment of mount on
 * narrower laptop viewports (see issue #1736).
 *
...
export function computeFitToWidthZoom(
  ...
  if (!naturalPageWidth || naturalPageWidth <= 0) return null;
