...
4. When asked to publish changes or open a PR, do not report success until GitHub confirms the PR number and URL. After pushing, verify with `gh pr view --json number,url,headRefName,state` or an equivalent GitHub API call; if no PR exists for the branch, create it before sending the final response.
